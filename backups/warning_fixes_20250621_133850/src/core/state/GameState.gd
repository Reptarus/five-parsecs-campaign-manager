@tool
extends Node
# BACKUP FILE - class_name removed to avoid conflicts
...
var current_campaign: FiveParsecsCampaign:
	...
	set(_value):
		_current_campaign = _value
		if _value:
			campaign_loaded.emit(_value) # warning: return value discarded (intentional)
...
func save_game(save_name: String, create_backup: bool = true) -> bool:
	if _save_operation_in_progress:
		...
		_save_queue.append({ # warning: return value discarded (intentional)
			"save_name": save_name,
	...
	save_completed.emit(true, "Game saved successfully") # warning: return value discarded (intentional)
	campaign_saved.emit() # warning: return value discarded (intentional)
	...
	if _save_retry_count < MAX_SAVE_ATTEMPTS:
		...
		if retry_success:
			...
			if move_success:
				_save_operation_in_progress = false
				last_save_time = Time.get_unix_time_from_system()
				save_completed.emit(true, "Game saved successfully after retry") # warning: return value discarded (intentional)
				campaign_saved.emit() # warning: return value discarded (intentional)
				return
	
	# All retries failed or not attempting retry
	_save_operation_in_progress = false
	save_completed.emit(false, error_message) # warning: return value discarded (intentional)
...
func _create_backup(file_path: String) -> bool:
	...
	if dir:
		...
		backup_created.emit(success, backup_name) # warning: return value discarded (intentional)
		return success
...
func advance_turn() -> void:
	if turn_number < max_turns:
		turn_number += 1
		turn_advanced.emit() # warning: return value discarded (intentional)
		state_changed.emit() # warning: return value discarded (intentional)
...
func add_resource(resource_type: GameEnums.ResourceType, amount: int) -> bool:
	...
	resources_changed.emit() # warning: return value discarded (intentional)
	return true
...
func add_quest(quest: Dictionary) -> bool:
	...
	quest_added.emit(quest) # warning: return value discarded (intentional)
	return true
...
func complete_quest(quest_id: String) -> bool:
	for quest in active_quests:
		if quest._id == quest_id:
			...
			quest_completed.emit(quest_id) # warning: return value discarded (intentional)
			return true
	return false
...
func set_location(location: Dictionary) -> void:
	...
	if not (location.id in visited_locations):
		visited_locations.append(location.id) # warning: return value discarded (intentional)
	state_changed.emit() # warning: return value discarded (intentional)
...
func _on_save_manager_save_completed(success: bool, message: String) -> void:
	if success:
		last_save_time = Time.get_unix_time_from_system()
	save_completed.emit(success, message) # warning: return value discarded (intentional)

func _on_save_manager_load_completed(success: bool, message: String) -> void:
	load_completed.emit(success, message) # warning: return value discarded (intentional)
...
func load_campaign(save_data: Dictionary) -> void:
	load_started.emit() # warning: return value discarded (intentional)
	
	if not save_data.has("campaign"):
		push_error("No campaign _data in save file")
		load_completed.emit(false, "No campaign _data in save file") # warning: return value discarded (intentional)
		return
	...
	campaign_loaded.emit(_current_campaign) # warning: return value discarded (intentional)
	state_changed.emit() # warning: return value discarded (intentional)
	load_completed.emit(true, "Campaign loaded successfully") # warning: return value discarded (intentional)
...
func save_campaign() -> Dictionary:
	...
	if not _current_campaign:
		...
		save_completed.emit(false, "No campaign to save") # warning: return value discarded (intentional)
		return {}
...
func has_equipment(equipment_type: Variant) -> bool:
	...
	if equipment_type is String:
		...
		if equipment_id == -1:
			push_warning("Invalid equipment _type string: " + str(equipment_type))
			return false
	else:
...
func get_market_price(resource_type: GameEnums.ResourceType) -> int:
	...
	if current_location:
		var location_type = current_location.get("_type", GameEnums.WorldTrait.NONE)
