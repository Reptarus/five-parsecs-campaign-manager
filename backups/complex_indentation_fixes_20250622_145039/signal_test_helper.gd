...
const ERROR_ARG_MISMATCH := "Signal '%s' argument count mismatch: expected % d butgot % d"
...
#
	if test_case.has_method("get_signal_emit_count"):
		actual_count = _case.get_signal_emit_count(emitter, signal_name)
...
#
	if test_case.has_method("get_signal_emit_count"):
		pass
		if emit_count > 0:
			actual_args = _case.get_signal_parameters(emitter, signal_name)
