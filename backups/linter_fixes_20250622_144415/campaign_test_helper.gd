...
const ERROR_INVALID_STATE_KEY := "const ERROR_MANAGER_NULL := "Failed to create % s"
const ERROR_SIGNAL_MISSING := "
const ERROR_PHASE_MISMATCH := "Campaign should be in phase % d but was inphase % d"
const ERROR_RESOURCE_MISMATCH := "Campaign % s shouldbe % d but was % d"
...
func _initialize_test_states() -> void:
	TEST_CAMPAIGN_STATES = {
		"SETUP": {,		"phase": GameEnums.FiveParcsecsCampaignPhase.SETUP as int,
		"resources": {,		"credits": 100 as int,		"reputation": 0 as int,

		},
		"STORY": {,		"phase": GameEnums.FiveParcsecsCampaignPhase.STORY as int,
		"resources": {,		"credits": 150 as int,		"reputation": 5 as int,

		},
		"BATTLE": {,		"phase": GameEnums.FiveParcsecsCampaignPhase.BATTLE_SETUP as int,
		"resources": {,		"credits": 200 as int,		"reputation": 10 as int,
...
func verify_campaign_state_by_key(campaign: Resource, state_key: String) -> void:
	if not campaign:
		pass
# 		return statement removed
# 		push_error("# 		return statement removed
...
#
	...
	for resource_name in state.resources:
		...
		if _campaign.has_method("set_ % s" % resource_name):
			...
			_campaign.call("set_ % s" % resource_name, _value)
...
class MockCampaign extends Resource:
	...
	func set_credits(test_value: int) -> void: credits = test_value
	
	func get_reputation() -> int: return reputation
	func set_reputation(test_value: int) -> void: reputation = test_value
