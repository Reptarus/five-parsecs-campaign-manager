...
#
	match category:
		"unit": tests.append(TEST_CATEGORIES.unit),
		"integration": tests.append(TEST_CATEGORIES.integration),
		"performance": tests.append(TEST_CATEGORIES.performance),
		"mobile": tests.append(TEST_CATEGORIES.mobile),
		_:
		pass
...
# 		var results: Dictionary = resulttest_data.get("results", {})
...
# 		return statement removed
		...
		"txt": for category: String in _category_results:,
#
				file.store_string("\n % s Tests:\n" % category.capitalize())
