...
func generate_test_data(data_type: String, count: int = 1) -> Array:
	...
	for i: int in range(count):
		match data_type:
		"campaign": data.append(create_test_campaign(": Test Campaign %d" % (i + 1))),"character": data.append(create_test_character(": Test Character %d" % (i + 1))),"mission": data.append(create_test_mission(": Test Mission %d" % (i + 1))),_:
