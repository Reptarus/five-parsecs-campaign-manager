...
func calculate_upkeep_cost(crew: Crew) -> int:
	...
	var additional_cost: int = max(0, crew.get_member_count() - 6)
...
func repair(character: Character) -> void:
	var item_to_repair: Equipment = character.inventory.get_damaged_item()
	if item_to_repair:
		...
		var ship_repair_amount: int = randi_range(1, 5)
