...
**Last Updated**: 2026-09-04 (**tablet deploys #14 + #15**, committed `c4317b993`). Deploy #14 walked the Sep 3 battle sprint on real hardware and found 11 defects, three of them in the sprint’s own phases. Deploy #15 closed the last two — and **both had been MISDIAGNOSED in #14**: T10-09 "touch-drag scroll is dead" was my own two-variable control (content swiped UP, scrollbar swiped DOWN, view already at the bottom) and is NOT a defect; T10-02’s ⚔ was never a missing glyph — `allow_system_fallback=true` is live, the glyph renders, it is just illegible at 16px. **Deploy #16 (Sep 4) then closed ALL ELEVEN on hardware** — including T10-06, where `progress.active_battle` is now structurally on disk mid-battle and the battle resumed intact across a real `am force-stop`. Two NEW findings opened: T11-01 (PreBattleUI footer clipped to ~13px in landscape, page will not scroll) and T11-02 (Seize panel states its threshold two ways). Detail: [docs/QA_STATUS_DASHBOARD.md](docs/QA_STATUS_DASHBOARD.md) § Sep 3-4. The Aug 8-14 tablet QA is still PAUSED — [docs/qa/PICKUP_2026-08-14.md](docs/qa/PICKUP_2026-08-14.md) is unchanged and still its pickup doc.
