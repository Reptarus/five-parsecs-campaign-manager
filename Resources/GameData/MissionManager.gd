...
func generate_standard_missions() -> Array[Mission]:
	...
	for i in range(num_missions):
		...
		mission.type = GlobalEnums.MissionType.ESCORT
...
func generate_patron_missions() -> Array[Mission]:
	...
	for patron in game_state.game_state.patrons:
		if randf() < 0.3:  # 30% chance for each patron to offer a mission
			var mission = Mission.new()
			mission.type = GlobalEnums.MissionType.PATROL
...
func generate_stealth_missions() -> Array[Mission]:
	...
	if randf() < 0.2:  # 20% chance to generate a stealth mission
		var mission = Mission.new()
		mission.type = GlobalEnums.MissionType.SABOTAGE
...
func _calculate_success_chance(mission: Mission) -> float:
	...
	match mission.type:
		GlobalEnums.MissionType.RETRIEVAL:
			base_chance = 0.4
		GlobalEnums.MissionType.ASSASSINATION:
			base_chance = 0.6
		GlobalEnums.MissionType.RESCUE:
			base_chance = 0.55
		GlobalEnums.MissionType.FRINGE_WORLD_STRIFE:
			base_chance = 0.45
	...
	match mission.objective:
		GlobalEnums.MissionObjective.FIGHT_OFF:
			crew_skill_modifier = (_calculate_average_combat_skill(game_state.game_state.current_ship.crew.map(func(cm): return cm as Character)) - 3) * 0.05
		GlobalEnums.MissionObjective.INFILTRATION:
			crew_skill_modifier = (_calculate_average_savvy(game_state.game_state.current_ship.crew.map(func(cm): return cm as Character)) - 3) * 0.05
		GlobalEnums.MissionObjective.ACQUIRE:
			crew_skill_modifier = (_calculate_average_savvy(game_state.game_state.current_ship.crew.map(func(cm): return cm as Character)) - 3) * 0.03
		...
		GlobalEnums.MissionObjective.DEFEND:
			crew_skill_modifier = (_calculate_average_combat_skill(game_state.game_state.current_ship.crew.map(func(cm): return cm as Character)) - 3) * 0.04
		GlobalEnums.MissionObjective.DELIVER:
			crew_skill_modifier = (_calculate_average_savvy(game_state.game_state.current_ship.crew.map(func(cm): return cm as Character)) - 3) * 0.03
		GlobalEnums.MissionObjective.ELIMINATE:
			crew_skill_modifier = (_calculate_average_combat_skill(game_state.game_state.current_ship.crew.map(func(cm): return cm as Character)) - 3) * 0.06
		GlobalEnums.MissionObjective.EXPLORE:
			crew_skill_modifier = (_calculate_average_savvy(game_state.game_state.current_ship.crew.map(func(cm): return cm as Character)) - 3) * 0.04
...
func get_mission_by_type(type: GlobalEnums.MissionType) -> Array[Mission]:
