...
func _get_rescued_units() -> Array:
    var container = mission_specific_inputs.get("rescue")
    if container:
        var rescue_count = container.get_node("RescueCountSpinBox")
...
func _check_extraction_point() -> bool:
    var container = mission_specific_inputs.get("rescue")
    if container:
        var extraction_check = container.get_node("ExtractionCheckBox")
        return extraction_check.button_pressed
    return false

func _get_destroyed_targets() -> Array:
    var container = mission_specific_inputs.get("sabotage")
    if container:
        var target_count = container.get_node("TargetCountSpinBox")
...
func _check_stealth_status() -> bool:
    var container = mission_specific_inputs.get("sabotage")
    if container:
        var stealth_check = container.get_node("StealthCheckBox")
        return stealth_check.button_pressed
    return false