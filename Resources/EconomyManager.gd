...
func calculate_upkeep_cost() -> int:
	...
	if game_state.current_location != null:
		...
		for i in range(location_traits.size()):
...
func apply_economic_event() -> void:
	...
	match event:
		...
		"Trade Embargo":
			# Logic for trade embargo is handled in trigger_global_event
			pass
		"Resource Shortage":
			# Logic for resource shortage is handled in trigger_global_event
			pass
		"Technological Breakthrough":
			# Logic for technological breakthrough is handled in trigger_global_event
			pass
