...
signal global_event_triggered(event_type: GameEnums.GlobalEvent)
...
func _on_campaign_event(event_type: GameEnums.GlobalEvent) -> void:
	match event_type:
...
func _load_quest_templates() -> void:
	if FileAccess.file_exists("res://Data/quest_templates.json"):
		var file := FileAccess.open("res://Data/quest_templates.json", FileAccess.READ)
		var json := JSON.new()
		var parse_result := json.parse(file.get_as_text())
		file.close()
...
func _setup_market_crash_quest(quest: StoryQuestData) -> void:
	...
	quest.objective = GameEnums.MissionObjective.RETRIEVE
...
func _setup_tech_breakthrough_quest(quest: StoryQuestData) -> void:
	...
	quest.objective = GameEnums.MissionObjective.SECURE_INTEL
	quest.story_point_reward = 2
	quest.rewards = {
		"credits": 1200,
		"reputation": 6,
...
func _create_generic_event(event_type: GameEnums.GlobalEvent) -> StoryQuestData:
	var event := StoryQuestData.new()
...
func _handle_generic_event(event_type: GameEnums.GlobalEvent) -> void:
	if not game_state:
		return
