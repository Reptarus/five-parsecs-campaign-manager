...
## Colors matching Deep Space theme (from BaseCampaignPanel)
const COLORS := {
	"info": Color("#2D5A7B"),      # Deep Space Blue
	"success": Color("#10B981"),   # Green
	"warning": Color("#D97706"),   # Orange
	"error": Color("#DC2626"),     # Red
	"background": Color("#252542"), # Card background
	"text": Color("#E0E0E0"),      # Primary text
	"border": Color("#3A3A5C")     # Card border
}
...
func _create_notification_panel(data: Dictionary) -> PanelContainer:
	...
	close_btn.add_theme_color_override("font_color", Color("#808080"))
