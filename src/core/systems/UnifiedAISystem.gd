...
func _process_default_behavior(unit: Character) -> Dictionary:
	return {
		"action": GlobalEnums.UnitAction.NONE,
...
func _calculate_threat_level(unit: Character) -> float:
	...
	if battlefield_manager and battlefield_manager.has_method("_world_to_grid"):
		grid_pos = battlefield_manager._world_to_grid(unit_pos)
...
func _evaluate_target_score(unit: Character, target: Character) -> float:
	...
	return (1.0 - health_ratio) * 0.4 + (1.0 - distance / 2.0) * 0.3 + threat_score * 0.3
...
func _calculate_objective_priority(unit: Character) -> float:
	...
	return maxf(0.0, 1.0 - (distance / 2.0))
