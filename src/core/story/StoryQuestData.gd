...
@export var mission_victory_conditions: Array[GameEnums.MissionVictoryType] = []
@export var contributes_to_campaign_victory: bool = true
...
func apply_effects(game_state: FiveParsecsGameState) -> void:
    if not game_state:
        return
...
func check_requirements(game_state: FiveParsecsGameState) -> bool:
    if not game_state:
        return false
...
func _setup_victory_conditions(battle_system: Node) -> void:
    # Set mission-specific victory conditions
    for condition in mission_victory_conditions:
        battle_system.add_victory_condition({
            "type": condition,
            "description": VictoryDescriptions.get_mission_description(condition),
            "completed": false
        })
    
    # Add any objective-specific victory conditions
    for objective in objectives:
        if objective.has("victory_condition"):
            battle_system.add_victory_condition({
                "type": objective.victory_condition,
                "description": VictoryDescriptions.get_mission_description(objective.victory_condition),
                "completed": false,
...
func _apply_resource_effects(game_state: FiveParsecsGameState, effects: Dictionary) -> void:
    for resource_type in effects:
        game_state.modify_resource(resource_type, effects[resource_type])

func _apply_relationship_effects(game_state: FiveParsecsGameState, effects: Dictionary) -> void:
    for faction_id in effects:
        game_state.modify_faction_relationship(faction_id, effects[faction_id])

func _apply_battle_modifiers(game_state: FiveParsecsGameState, modifiers: Dictionary) -> void:
    for modifier_type in modifiers:
        game_state.apply_battle_modifier(modifier_type, modifiers[modifier_type])

func _check_resource_requirements(game_state: FiveParsecsGameState, requirements: Dictionary) -> bool:
    for resource_type in requirements:
        if not game_state.has_sufficient_resource(resource_type, requirements[resource_type]):
            return false
    return true

func _check_relationship_requirements(game_state: FiveParsecsGameState, requirements: Dictionary) -> bool:
    for faction_id in requirements:
        if not game_state.has_sufficient_relationship(faction_id, requirements[faction_id]):
            return false
    return true

func _check_battle_rating_requirement(game_state: FiveParsecsGameState, required_rating: int) -> bool:
...
func serialize() -> Dictionary:
    return {
        ...
        "mission_victory_conditions": mission_victory_conditions,
        "contributes_to_campaign_victory": contributes_to_campaign_victory
    }
...
func deserialize(data: Dictionary) -> void:
    ...
    mission_victory_conditions = data.get("mission_victory_conditions", [])
    contributes_to_campaign_victory = data.get("contributes_to_campaign_victory", true)

func check_victory_conditions(battle_results: Dictionary) -> bool:
    # Check mission-level victory conditions
    for condition in mission_victory_conditions:
        if not _check_mission_victory_condition(condition, battle_results):
            return false
            
    # Check objective-specific victory conditions
    for objective in objectives:
        if objective.has("victory_condition") and not objective.completed:
            return false
            
    return true

func _check_mission_victory_condition(condition: int, results: Dictionary) -> bool:
    match condition:
        GameEnums.MissionVictoryType.ELIMINATION:
            return results.get("enemies_eliminated", 0) >= results.get("total_enemies", 1)
        GameEnums.MissionVictoryType.EXTRACTION:
            return results.get("reached_extraction", false)
        GameEnums.MissionVictoryType.SURVIVAL:
            return results.get("turns_survived", 0) >= results.get("required_turns", 1)
        GameEnums.MissionVictoryType.CONTROL_POINTS:
            return results.get("points_controlled", 0) >= results.get("required_points", 1)
        GameEnums.MissionVictoryType.OBJECTIVE:
            return results.get("objectives_completed", 0) >= results.get("required_objectives", 1)
        _:
            push_warning("Unknown victory condition type: %s" % condition)
            return false