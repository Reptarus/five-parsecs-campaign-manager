...
func _connect_character_signals() -> void:
	...
	if character_manager and character_manager.has_method("has_signal") and character_manager.has_signal("character_added"):
		# Disconnect if already connected to prevent duplicates
		@warning_ignore("unsafe_method_access")
		if character_manager and character_manager.has_method("is_connected") and character_manager.is_connected("character_added", _on_character_added):
			@warning_ignore("return_value_discarded")
	# Production-quality signal connection with proper type safety
	var char_manager := get_node_or_null("/root/CharacterManager")
	if not char_manager:
		push_error("EquipmentManager: CharacterManager not found in scene tree")
		return
	
	# Safe signal connections with error handling
	if char_manager.has_signal("character_added"):
		var connect_result: Error = char_manager.character_added.connect(_on_character_added)
		if connect_result != OK:
			push_error("EquipmentManager: Failed to connect character_added signal")
	
	if char_manager.has_signal("character_removed"):
		var connect_result: Error = char_manager.character_removed.connect(_on_character_removed)
		if connect_result != OK:
			push_error("EquipmentManager: Failed to connect character_removed signal")

func _safe_get_string(data: Dictionary, key: String, default: String = "") -> String:
	"""Production-safe string extraction with type validation"""
	var value = data.get(key, default)
	return value if value is String else default

func _safe_get_int(data: Dictionary, key: String, default: int = 0) -> int:
	"""Production-safe integer extraction with type validation"""
	var value = data.get(key, default)
	return value if value is int else default

## PRODUCTION TYPE SAFETY UTILITIES
func _validate_equipment_data(data: Dictionary) -> bool:
	"""Comprehensive validation for production reliability"""
	return data.has("id") and data.has("name") and not _safe_get_string(data, "id").is_empty()

func _emit_equipment_acquired(equipment: Dictionary) -> void:
	"""Type-safe signal emission"""
	if equipment_acquired.get_connections().size() > 0:
		equipment_acquired.emit(equipment)

func _emit_equipment_removed(character_id: String, equipment_id: String) -> void:
	"""Type-safe signal emission with validation"""
	if equipment_removed.get_connections().size() > 0:
...
func _get_safe_current_battle() -> Dictionary:
	if not battle_results_manager:
		return {}
	if not battle_results_manager and battle_results_manager.has_method("get_current_battle"):
		return {}
...
func add_equipment(equipment_data: Dictionary) -> bool:
	...
	if new_id.is_empty():
		push_error("EquipmentManager: Equipment ID cannot be empty")
		return false

	for item: Dictionary in _equipment_storage:
		var existing_id := _safe_get_string(item, "id")
		if existing_id == new_id:
			push_error("EquipmentManager: Equipment with ID already exists: %s" % new_id)
			return false
...
func get_equipment(equipment_id: String) -> Dictionary:
	if equipment_id.is_empty():
		push_warning("EquipmentManager: Empty equipment ID provided")
		return {}
	
	for item: Dictionary in _equipment_storage:
		var item_id := _safe_get_string(item, "id")
...
func get_equipment_by_category(category: int) -> Array:
	...
	for item: Dictionary in _equipment_storage:
		if item.get("category", null) == category:
...
func sell_equipment(equipment_id: String) -> int:
	...
	var sell_value: int = int(base_value * (condition / 10.0) * 0.5) # 50% of value
...
func generate_battle_loot(difficulty: int, success: bool = true) -> Array:
	...
	if current_battle.get("mission_type", null) == GlobalEnums.MissionType.BLACK_ZONE:
		_add_loot_item(loot_items, _roll_on_loot_table())
...
func _generate_weapon_by_rulebook() -> Dictionary:
	...
	var weapon_type: int = GlobalEnums.WeaponType.BASIC

	if weapon_type_roll <= 40:
		weapon_type = GlobalEnums.WeaponType.PISTOL
	elif weapon_type_roll <= 75:
		weapon_type = GlobalEnums.WeaponType.RIFLE
	elif weapon_type_roll <= 90:
		weapon_type = GlobalEnums.WeaponType.ADVANCED
	else:
		weapon_type = GlobalEnums.WeaponType.HEAVY
...
func _generate_armor_by_rulebook() -> Dictionary:
	...
	if armor_type_roll <= 60:
		armor_type = GlobalEnums.ArmorType.LIGHT
	elif armor_type_roll <= 90:
		armor_type = GlobalEnums.ArmorType.MEDIUM
	else:
		armor_type = GlobalEnums.ArmorType.HEAVY
	...
	var unique_items: Array[Dictionary] = [
		{
			...
			"type": "weapon",
			"stats": {
				"weapon_type": GlobalEnums.WeaponType.SPECIAL,
				"damage": 5,
				"range": 3
			}
		},
		{
			...
			"type": "armor",
			"stats": {
				"armor_type": GlobalEnums.ArmorType.POWERED,
				"protection": 4
			}
		},
		{
...
func _can_character_use_equipment(character: Variant, equipment_data: Dictionary) -> bool:
	...
	if category == EquipmentCategory.WEAPON:
		@warning_ignore("unsafe_method_access")
		var weapon_type: int = equipment_data.get("weapon_type", GlobalEnums.WeaponType.NONE)
		@warning_ignore("unsafe_method_access")
		var char_class: int = character.get("character_class", GlobalEnums.CharacterClass.NONE)

		# Some special weapons might be restricted
		if weapon_type == GlobalEnums.WeaponType.HEAVY:
	...
	if category == EquipmentCategory.ARMOR:
		@warning_ignore("unsafe_method_access")
		var armor_type: int = equipment_data.get("armor_type", GlobalEnums.ArmorType.NONE)
		@warning_ignore("unsafe_method_access")
		var char_toughness: int = character.get("toughness", 1)

		# Heavy armor requires higher toughness
		if armor_type == GlobalEnums.ArmorType.HEAVY and char_toughness < 3:
			return false
...
func _update_character_with_equipment(character_id: String) -> void:
	...
	for equipment_id: String in equipment_list:
		...
		match equipment.get("category", null):
...
func _generate_random_item(difficulty: int) -> Dictionary:
	...
	match item_type:
		0: # Weapon
			...
			if difficulty >= 3:
				_add_weapon_type(weapon_types, GlobalEnums.WeaponType.ADVANCED)

			# Add heavy weapons at high difficulties
			if difficulty >= 5:
				_add_weapon_type(weapon_types, GlobalEnums.WeaponType.HEAVY)

			var weapon_type: int = weapon_types[randi() % weapon_types.size()]
			var damage: int = 1 + (randi() % (1 + int(difficulty / 2.0)))
		...
		1: # Armor
			...
			if difficulty >= 4:
				_add_armor_type(armor_types, GlobalEnums.ArmorType.HEAVY)

			var armor_type: int = armor_types[randi() % armor_types.size()]
			var protection: int = 1 + (randi() % (1 + int(difficulty / 2.0)))
...
func upgrade_weapon(weapon_id: String, upgrade_type: String) -> bool:
	...
	if weapon.is_empty() or weapon.get("category", null) != EquipmentCategory.WEAPON:
		return false
	...
	@warning_ignore("unsafe_method_access")
	if game_state.get_credits() < upgrade_cost:
		return false
...
func upgrade_armor(armor_id: String, upgrade_type: String) -> bool:
	...
	if armor.is_empty() or armor.get("category", null) != EquipmentCategory.ARMOR:
		return false
...
func _calculate_weapon_upgrade_cost(weapon: Dictionary, upgrade_type: String) -> int:
	...
	var weapon_type: int = weapon.get("_type", GlobalEnums.WeaponType.BASIC)
	...
	match upgrade_type:
		...
		"condition":
			...
			base_cost = int(50.0 * (1.0 - condition / 10.0))
...
func _calculate_armor_upgrade_cost(armor: Dictionary, upgrade_type: String) -> int:
	...
	var armor_type: int = armor.get("armor_type", GlobalEnums.ArmorType.LIGHT)
	...
	match upgrade_type:
		...
		"condition":
			...
			base_cost = int(75.0 * (1.0 - condition / 10.0))
