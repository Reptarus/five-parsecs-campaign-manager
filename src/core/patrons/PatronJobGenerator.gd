...
func _load_job_templates() -> void:
	...
	if dm and dm.has_method("load_json_file"):
		...
		if not job_results.is_empty():
			for roll_key in job_results:
				var entry: Dictionary = job_results[roll_key]
				var job_type: String = entry.get("job_type", "").to_upper()
				if not job_type.is_empty() and not job_templates.has(job_type):
					job_templates[job_type] = {
						"titles": [entry.get("description", job_type)],
						"descriptions": [entry.get("description", "")],
						"base_payment": entry.get("base_pay", 5),
						"danger_range": [entry.get("danger_level", 1), entry.get("danger_level", 1) + 1],
						"difficulty_range": [1, entry.get("danger_level", 1) + 1],
	...
	_load_job_templates_hardcoded()

func _load_job_templates_hardcoded() -> void:
	job_templates = {
		"ESCORT": {
