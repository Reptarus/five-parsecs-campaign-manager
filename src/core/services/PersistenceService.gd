...
# Autoload References - GameState is the canonical campaign state source
# Accessed via get_node_or_null("/root/GameState")
...
func load_campaign(file_path: String) -> Dictionary:
	...
	# Restore QOL autoload state from save data
	if save_data.has("qol_data"):
		var _qol: Dictionary = save_data.qol_data
		var _journal = get_node_or_null("/root/CampaignJournal")
		if _journal and _journal.has_method("load_from_save"):
			_journal.load_from_save(save_data)
		var _checklist = get_node_or_null("/root/TurnPhaseChecklist")
		if _checklist and _checklist.has_method("load_from_save"):
			_checklist.load_from_save(save_data)
		var _npc_tracker = get_node_or_null("/root/NPCTracker")
		if _npc_tracker and _npc_tracker.has_method("deserialize"):
			_npc_tracker.deserialize(_qol.get("npc_tracker", {}))
		var _legacy = get_node_or_null("/root/LegacySystem")
...
func quick_save() -> bool:
	...
	if not gs:
		save_error.emit("GameState not available for quick save")
		return false

	var campaign_data = gs.serialize()
...
func _prepare_save_data(campaign_data: Dictionary = {}) -> Dictionary:
	...
	var _cm = get_node_or_null("/root/CampaignManager")
	@warning_ignore("unsafe_method_access")
	save_data["campaign_manager_state"] = _cm.serialize() if _cm else {}
	
	# Collect QOL autoload state (journal, checklist, NPC tracker, legacy)
	var qol_data := {}
	var _journal = get_node_or_null("/root/CampaignJournal")
	if _journal and _journal.has_method("save_to_dict"):
		qol_data["journal"] = _journal.save_to_dict()
	var _checklist = get_node_or_null("/root/TurnPhaseChecklist")
	if _checklist and _checklist.has_method("save_to_dict"):
		qol_data["checklist_settings"] = _checklist.save_to_dict()
	var _npc_tracker = get_node_or_null("/root/NPCTracker")
	if _npc_tracker and _npc_tracker.has_method("serialize"):
		qol_data["npc_tracker"] = _npc_tracker.serialize()
	var _legacy = get_node_or_null("/root/LegacySystem")
	if _legacy and _legacy.has_method("serialize"):
		qol_data["hall_of_fame"] = _legacy.serialize()
	if not qol_data.is_empty():
		save_data["qol_data"] = qol_data
