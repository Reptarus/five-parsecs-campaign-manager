...
signal battle_ended()
signal escalation_check_needed(round_number: int)
...
func _check_and_trigger_battle_event() -> void:
	...
	# Escalation check for rounds > 4 (Five Parsecs extended battle rules)
	if _current_round > 4:
		escalation_check_needed.emit(_current_round)
