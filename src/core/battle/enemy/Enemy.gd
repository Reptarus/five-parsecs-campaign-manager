@tool
extends Resource
class_name BattleEnemyResource
...
func _connect_signals() -> void:
	...
	if _base_enemy:
		...
		armor = _base_enemy.get("armor") if _base_enemy.get("armor") != null else 5.0
		movement_range = _base_enemy.get("movement_range") if _base_enemy.get("movement_range") != null else 4.0
		weapon_range = _base_enemy.get("weapon_range") if _base_enemy.get("weapon_range") != null else 1.0
...
func _notification(what: int) -> void:
	...
	if what == NOTIFICATION_POSTINITIALIZE:
		...
		if resource_path.is_empty():
			...
			resource_path = "res://tests/generated/battle_enemy_resource_%d.tres" % [Time.get_unix_time_from_system()]
