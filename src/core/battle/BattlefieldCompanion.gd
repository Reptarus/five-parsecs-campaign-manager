...
var tier_controller: BattleTierControllerClass = null
...
func _initialize_core_systems() -> void:
	...
	# Initialize tier controller (controls which components/features are active)
	tier_controller = BattleTierControllerClass.new()
	tier_controller.tier_changed.connect(_on_tier_changed)
...
func _on_tier_changed(old_tier: int, new_tier: int) -> void:
	## Handle tracking tier change
	session_data["tracking_tier"] = new_tier
	if save_session_data:
		_save_session_state()

## Set tracking tier for this battle session.
## Use force=true at battle start to allow any tier.
func set_tracking_tier(tier: int, force: bool = false) -> bool:
	if not tier_controller:
		return false
	return tier_controller.set_tier(tier, force)

## Get current tracking tier.
func get_tracking_tier() -> int:
	if not tier_controller:
		return 0
	return tier_controller.get_current_tier()
...
func _save_session_state() -> void:
	...
	var save_data := {
		...
		"tracking_tier": tier_controller.serialize() if tier_controller else {},
