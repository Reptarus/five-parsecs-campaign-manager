...
func _find_valid_objective_position(objective: Dictionary, placement_rules: Dictionary) -> Vector2i:
    var min_distance = placement_rules.get("min_distance_from_features", 2)
    var max_attempts = 30
    
    for attempt in range(max_attempts):
        var x = rng.randi_range(2, battlefield_grid[0].size() - 3)
        var y = rng.randi_range(2, battlefield_grid.size() - 3)
        var position = Vector2i(x, y)
...
func _generate_deployment_zones() -> Dictionary:
    ...
    for y in range(grid_height):
        # Crew deploys in leftmost 4 columns
        for x in range(min(4, grid_width)):
            crew_zone.append(Vector2i(x, y))
        
        # Enemies deploy in rightmost 4 columns
        for x in range(max(0, grid_width - 4), grid_width):
