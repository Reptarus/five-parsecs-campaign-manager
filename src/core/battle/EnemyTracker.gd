...
var status_colors = {
	"active": Color.GREEN,
	"inactive": UIColors.COLOR_TEXT_SECONDARY,
...
func activate_next_enemy() -> String:
	...
	var enemy_id: String = activation_order[current_activation_index]
...
func _create_enemy_display(enemy_info: Dictionary) -> void:
	...
	var enemy_panel: Panel = _create_simple_enemy_panel(enemy_info)
...
func _remove_enemy_display(enemy_id: String) -> void:
	var panel: Node = enemy_list.get_node_or_null("Enemy_" + enemy_id)
	if panel:
		panel.queue_free()
...
func _update_enemy_display(enemy_id: String) -> void:
	...
	# Update health label
	var health_label: Label = panel.get_node_or_null("VBoxContainer/HealthLabel")
	if health_label:
		health_label.text = "Health: " + str(enemy.health) + "/" + str(enemy.max_health)

	# Update status label and color
	var status_label: Label = panel.get_node_or_null("VBoxContainer/StatusLabel")
	if status_label:
...
func _update_activation_controls() -> void:
	if turn_complete:
		...
		var next_enemy: Dictionary = get_next_enemy()
