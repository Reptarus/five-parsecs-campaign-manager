...
func set_battlefield_manager(manager: Variant) -> void:
...
func start_ai_turn() -> void:
	ai_turn_started.emit() # Signal emits void, don't capture return value
	...
	ai_turn_ended.emit() # Signal emits void, don't capture return value
	current_unit = null
...
func _get_movement_options(unit: CharacterUnit) -> Array:
	...
	if battlefield_manager and battlefield_manager.has_method("_world_to_grid"):
		current_cell = battlefield_manager._world_to_grid(current_pos)

	# Check cells in movement range
	for x: int in range(current_cell.x - move_range, current_cell.x + move_range + 1):
		for y: int in range(current_cell.y - move_range, current_cell.y + move_range + 1):
			var cell = Vector2i(x, y)

			# Skip if outside battlefield or current position
			var is_valid_position: bool = false
			if battlefield_manager and battlefield_manager.has_method("_is_valid_grid_position"):
				is_valid_position = battlefield_manager._is_valid_grid_position(cell)

			if not is_valid_position or cell == current_cell:
				continue
			...
			if battlefield_manager and battlefield_manager.has_method("_grid_to_world"):
				world_pos = battlefield_manager._grid_to_world(cell)

			# Get terrain information
			var terrain_type: int = 0
			if battlefield_manager and battlefield_manager.has_method("get_terrain_type"):
				terrain_type = battlefield_manager.get_terrain_type(world_pos)
			...
			if battlefield_manager and battlefield_manager.has_method("get_cover_value"):
				cover_value = battlefield_manager.get_cover_value(world_pos)
...
func _is_cell_occupied(cell: Vector2i) -> bool:
	...
	for unit in enemy_units + player_units:
		...
		var unit_cell: Vector2i = Vector2i.ZERO
		if battlefield_manager and battlefield_manager.has_method("_world_to_grid"):
			unit_cell = battlefield_manager._world_to_grid(unit.global_position)
		if unit_cell == cell:
			return true
...
func _has_line_of_sight(from_unit: CharacterUnit, to_unit: CharacterUnit) -> bool:
	...
	if battlefield_manager and battlefield_manager.has_method("_world_to_grid"):
		from_pos = battlefield_manager._world_to_grid(from_unit.global_position)
	var to_pos: Vector2i = Vector2i.ZERO
	if battlefield_manager and battlefield_manager.has_method("_world_to_grid"):
		to_pos = battlefield_manager._world_to_grid(to_unit.global_position)
	...
	while x0 != x1 or y0 != y1:
		...
		if Vector2i(x0, y0) != from_pos and Vector2i(x0, y0) != to_pos:
			var terrain_type: int = 0
			if battlefield_manager and battlefield_manager.has_method("get_terrain_type"):
				terrain_type = battlefield_manager.get_terrain_type(
					battlefield_manager._grid_to_world(Vector2i(x0, y0)) if battlefield_manager.has_method("_grid_to_world") else Vector2.ZERO
				)
...
func _calculate_attack_score(unit: CharacterUnit, target: CharacterUnit) -> float:
	...
	for player_unit in player_units:
		var typed_player_unit: Variant = player_unit
...
func _calculate_move_score(unit: CharacterUnit, option: Dictionary) -> float:
	...
	var group_factor: float = 0.0
	for ally in enemy_units:
		var typed_ally: Variant = ally
