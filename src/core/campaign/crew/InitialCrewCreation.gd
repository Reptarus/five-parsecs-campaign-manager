...
class CrewRelationshipManagerClass extends Node:
	...
	func add_crew_member(_member: Variant) -> void:
		pass
...
func _update_button_states() -> void:
	...
	add_crew_member_button.disabled = not has_captain or (safe_call_method(current_crew, "size") as int) >= campaign_config.get("crew_size", DEFAULT_CREW_SIZE)
...
func _on_add_crew_member_pressed() -> void:
	...
	if (safe_call_method(current_crew, "size") as int) >= campaign_config.get("crew_size", DEFAULT_CREW_SIZE):
...
func _validate_crew() -> bool:
	if (safe_call_method(current_crew, "is_empty") == true):
		return false
	...
	return (safe_call_method(current_crew, "size") as int) == required_size
...
func _on_character_slot_pressed(slot_index: int) -> void:
	...
	if slot_index < (safe_call_method(current_crew, "size") as int):
...
func _on_crew_member_selected(index: int) -> void:
	if index >= 0 and index < (safe_call_method(current_crew, "size") as int):
