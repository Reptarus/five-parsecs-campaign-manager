...
func _process_rival_status() -> void:
	...
	# Core Rules p.119: Rival resolution depends on whether opponent was existing Rival
	...
	for enemy in defeated_enemies:
		if enemy.get("is_rival", false):
			...
			if rival_id != "" and held_field:
				# Held Field vs existing Rival: D6 +1(tracked) +1(killed Unique); 4+ = removed
				var removal_roll = _roll_rival_removal(rival_id)
				if removal_roll >= 4: # Core Rules p.119: 4+ threshold
	...
	# Core Rules p.119: Held Field vs non-Rival opponent → D6, on 1 = new Rival
	if held_field and not fought_existing_rival:
		var new_rival_roll: int = randi_range(1, 6)
		if new_rival_roll == 1:
			var new_rival_id: String = _create_new_rival_from_battle()
			if new_rival_id != "":
...
func _roll_rival_removal(rival_id: String) -> int:
	## Roll to determine if rival is permanently removed (Core Rules p.119)
	## D6 + modifiers; 4+ = rival removed permanently
	var base_roll: int = randi_range(1, 6)
	var modifiers: int = 0

	# +1 if crew used Track task to locate this rival
	...
	# +1 if killed a Unique Individual during battle
	for enemy in defeated_enemies:
...
func _create_new_rival_from_battle() -> String:
	## Create a new Rival from a non-Rival opponent after Holding Field (Core Rules p.119)
	## D6 roll of 1 = new Rival on current planet
	var game_state = _game_state
	...
	var planet_id: String = battle_result.get("planet_id", "")
	...
	var new_rival: Dictionary = {
		...
		"planet_id": planet_id,
		"threat_level": 1,
		"created_turn": battle_result.get("turn", 0),
		...
		if travel_roll >= 4:
			...
			if requires_new_world:
				...
				if GameState.has_method("set_quest_requires_travel"):
					GameState.set_quest_requires_travel(true, true)
			else:
				print("PostBattlePhase: Quest finale requires travel (roll %d)" % travel_roll)
				if GameState.has_method("set_quest_requires_travel"):
					GameState.set_quest_requires_travel(true, false)
		else:
...
func _roll_battlefield_find() -> Dictionary:
	...
	## Uses LootSystemConstants for accurate D100 ranges
	var find_roll: int = randi_range(1, 100)

	# Use LootSystemConstants if available for accurate D100 category lookup
	...
	if LootConstants and LootConstants.has_method("get_battlefield_finds_category"):
		...
		# Map category enum to result dictionary
		# BattlefieldFindsCategory: WEAPON=0, CONSUMABLE=1, QUEST_RUMOR=2, SHIP_PART=3,
		#   TRINKET=4, DEBRIS=5, VITAL_INFO=6, NOTHING=7
		match category:
			0: # WEAPON
	...
	# Fallback D100 ranges matching LootSystemConstants battlefield finds table
	if find_roll <= 15:
		return {"type": "weapon", "roll": find_roll, "description": "Weapon found on battlefield", "subtable": "weapon"}
	elif find_roll <= 25:
		return {"type": "consumable", "roll": find_roll, "description": "Consumable supplies found"}
...
func _process_experience() -> void:
	...
	for participant in crew_participants:
		...
		var xp_earned: int = _calculate_crew_xp(crew_id)
		if xp_earned > 0:
...
func _is_crew_member_bot(crew_id: String) -> bool:
	...
	if GameState and GameState.has_method("get_crew_member"):
		var crew_member: Variant = GameState.get_crew_member(crew_id)
		if crew_member:
	...
	if game_state_manager and game_state_manager.has_method("get_crew_member"):
		var gsm_crew_member: Variant = game_state_manager.get_crew_member(crew_id)
		if gsm_crew_member and gsm_crew_member is Dictionary:
			return gsm_crew_member.get("is_bot", false)
...
func _get_training_course(course_name: String) -> Dictionary:
	## Returns training course data for Advanced Training (Core Rules p.125)
	## 7 courses available per rulebook + basic training fallback
	...
	return courses.get(course_name, {"cost": 1, "effect": "xp_bonus", "description": "Basic training"})
...
func attempt_training_enrollment(crew_id: String, course: String, available_credits: int) -> Dictionary:
	...
	# Get course data
	var course_data: Dictionary = _get_training_course(course)
...
func _get_campaign_event(roll: int) -> Dictionary:
	## Get campaign event based on D100 roll - loads from JSON data file (Core Rules p.126-128)
	var json_path: String = "res://data/campaign_tables/campaign_events.json"
	if not FileAccess.file_exists(json_path):
		push_warning("PostBattlePhase: campaign_events.json not found at %s" % json_path)
		return {"type": "none", "name": "No Event", "description": "Nothing significant occurs"}
	var file: FileAccess = FileAccess.open(json_path, FileAccess.READ)
	if not file:
		push_warning("PostBattlePhase: Failed to open campaign_events.json")
		return {"type": "none", "name": "No Event", "description": "Nothing significant occurs"}
	var json: JSON = JSON.new()
	var parse_result: int = json.parse(file.get_as_text())
	file.close()
	if parse_result != OK:
		push_warning("PostBattlePhase: Failed to parse campaign_events.json")
		return {"type": "none", "name": "No Event", "description": "Nothing significant occurs"}
...
func _get_character_event() -> Dictionary:
	## Get character event for random non-Bot non-Soulless crew member (Core Rules p.128-130)
	## Precursors roll twice and pick either result (handled in _process_character_event)
	...
	if not FileAccess.file_exists(json_path):
		push_warning("PostBattlePhase: character_events.json not found at %s" % json_path)
		return {"type": "none", "name": "No Event"}
	var file: FileAccess = FileAccess.open(json_path, FileAccess.READ)
	if not file:
		push_warning("PostBattlePhase: Failed to open character_events.json")
		return {"type": "none", "name": "No Event"}
	var json: JSON = JSON.new()
	var parse_result: int = json.parse(file.get_as_text())
	file.close()
	if parse_result != OK:
		push_warning("PostBattlePhase: Failed to parse character_events.json")
		return {"type": "none", "name": "No Event"}
	var data: Dictionary = json.data
	var entries: Array = data.get("entries", [])
	for entry in entries:
...
func _mark_planet_lost(planet_id: String) -> void:
	...
	var game_state: Variant = _game_state
	if game_state and game_state.current_campaign:
		var campaign: Variant = game_state.current_campaign
...
func _add_quest_rumor() -> void:
	## Add a quest rumor to campaign (Core Rules p.120)
	var game_state_ref = _game_state
	if game_state_ref and game_state_ref.current_campaign:
		var campaign = game_state_ref.current_campaign
		if campaign is Dictionary:
			var rumors: Array = campaign.get("rumors", [])
...
func _add_rival(rival_name: String) -> void:
	## Add a rival to campaign (Core Rules p.119)
	var game_state_ref = _game_state
	if game_state_ref and game_state_ref.current_campaign:
		var campaign = game_state_ref.current_campaign
		if campaign is Dictionary:
			var rivals: Array = campaign.get("rivals", [])
...
func _remove_random_patron() -> void:
	...
	if game_state_ref and game_state_ref.current_campaign:
		...
		if campaign is Dictionary:
			...
			if patrons.size() > 0:
				var idx: int = randi() % patrons.size()
				var removed: Variant = patrons[idx]
				patrons.remove_at(idx)
				campaign["patrons"] = patrons
				print("PostBattlePhase: Removed patron: %s" % str(removed.get("name", "Unknown") if removed is Dictionary else "Unknown"))
			else:
				print("PostBattlePhase: No patrons to remove")
...
func _add_random_equipment_to_stash() -> void:
	## Add a random equipment item to ship stash
	var game_state_ref = _game_state
	if game_state_ref and game_state_ref.current_campaign:
		var campaign = game_state_ref.current_campaign
		...
		if campaign is Dictionary:
			var equipment: Array = campaign.get("equipment", campaign.get("equipment_data", {}).get("equipment", []))
			equipment.append(item)
		print("PostBattlePhase: Added %s to stash" % item.get("name", "Unknown"))
...
func apply_campaign_event_effect(event_title: String) -> String:
	...
	match event_title:
		...
		"Valuable Find":
			...
			if GameStateManager:
				GameStateManager.add_credits(credits_val)
