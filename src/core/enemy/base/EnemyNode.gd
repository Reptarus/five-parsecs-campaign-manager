...
var is_leader_state: bool = false
...
func _ready() -> void:
	...
	if not enemy_data:
		...
		# Create a default enemy_data when missing
		...
		# These properties are now explicitly defined in EnemyData
		enemy_data.movement_range = 3
		enemy_data.weapon_range = 1
		
		# Check properties that might not exist in all versions of EnemyData
		if "name" in enemy_data:
			enemy_data.name = "Default Enemy"
		if "id" in enemy_data:
			enemy_data.id = "default_enemy_" + str(get_instance_id())
		# Add behavior and stance properties if they exist
		if "behavior" in enemy_data:
			enemy_data.behavior = 0 # Default to passive
		if "stance" in enemy_data:
			enemy_data.stance = 0 # Default to neutral
	
	# Now initialize with the data
	if enemy_data:
		...
		# Store a back-reference using weak reference to prevent cycles
		if enemy_data.has_method("set_parent_node"):
			enemy_data.set_parent_node(self)
...
func initialize(data: EnemyData) -> bool:
	...
	if enemy_data.has("initial_position") and enemy_data.initial_position != Vector2.INF: # Example property
		global_position = enemy_data.initial_position
...
func has_status_effect(effect_name: String) -> bool:
	return enemy_data.has_status_effect(effect_name) if enemy_data else false
...
func _physics_process(delta: float) -> void:
	...
	if navigation_agent and not navigation_agent.is_navigation_finished():
		...
		var desired_velocity = direction * (enemy_data.speed if enemy_data.has("speed") else 100.0) # Get speed from data if available
...
func can_move() -> bool:
	...
	# Should likely check enemy_data for status effects preventing movement
	return not is_dead() and (not enemy_data or not enemy_data.has_status_effect("stunned")) # Example

func can_attack() -> bool:
	return not is_dead() and (not enemy_data or not enemy_data.has_status_effect("disarmed")) # Example
...
# --- Leader functionality for tactical combat ---
func set_as_leader(value: bool) -> void:
	is_leader_state = value

func is_leader() -> bool:
	return is_leader_state

# Function to set current target for tactical combat
func set_current_target(target_node) -> bool:
...
func cleanup() -> void:
	...
	if enemy_data:
		...
		# Clear reference to parent in enemy_data
		if enemy_data.has_method("set_parent_node"):
			enemy_data.set_parent_node(null)
...
# Override _exit_tree to clean up resources
