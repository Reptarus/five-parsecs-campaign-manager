...
func _generate_planet(sector_name: String, coordinates: Vector2) -> Planet:
    ...
    planet.world_trait = randi() % GlobalEnums.WorldTrait.size()
    planet.threat_type = randi() % GlobalEnums.ThreatType.size()
    planet.strife_level = randi() % GlobalEnums.StrifeType.size()
    planet.instability = randi() % GlobalEnums.FringeWorldInstability.size()
