...
const VICTORY_CATEGORIES = {
	"Wealth": {
		"WEALTH_5000": {
			"type": GameEnums.CampaignVictoryType.WEALTH_GOAL,
			"name": "Wealthy Crew",
	...
	"Reputation": {
		"REPUTATION_HIGH": {
			"type": GameEnums.CampaignVictoryType.REPUTATION_GOAL,
			"name": "Renowned Crew",
	...
	"Faction": {
		"FACTION_DOMINANCE": {
			"type": GameEnums.CampaignVictoryType.FACTION_DOMINANCE,
	...
	"Story": {
		"STORY_COMPLETE": {
			"type": GameEnums.CampaignVictoryType.STORY_COMPLETE,
...
func select_victory_condition(condition_key: String) -> void:
	...
	if condition_key == "CUSTOM":
		...
		victory_selected.emit(GameEnums.CampaignVictoryType.NONE, custom_data)
	else:
