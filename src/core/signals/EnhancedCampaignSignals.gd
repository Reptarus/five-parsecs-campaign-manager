...
## NO enterprise bloat - just the essential functionality WorldInfoPanel needs
...
func connect_signal_safely(signal_name: String, target: Object, method_name: String) -> bool:
	...
	if not target:
		push_warning("EnhancedCampaignSignals: Target is null for signal '%s'" % signal_name)
		return false
	
	if not target.has_method(method_name):
		push_warning("EnhancedCampaignSignals: Target method '%s' not found" % method_name)
		return false
	...
	if error != OK:
		push_error("EnhancedCampaignSignals: Failed to connect signal '%s': %s" % [signal_name, error])
		return false
...
func emit_safe_signal(signal_name: String, args: Array = []) -> bool:
	...
	match args.size():
		...
		_: 
			push_warning("EnhancedCampaignSignals: Too many arguments for signal '%s'" % signal_name)
			return false
