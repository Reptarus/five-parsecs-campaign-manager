...
func _generate_linear_obstacles() -> void:
	...
	for _i in range(num_obstacles):
		...
		var pos := Vector2i(
			randi_range(0, terrain_system.grid_size.x - (1 if horizontal else length)),
			randi_range(0, terrain_system.grid_size.y - (length if horizontal else 1))
		)
...
func _place_terrain_feature(pos: Vector2i, size: Vector2i, type: int) -> void:
	if shapes_2d:
		var shape = shapes_2d.create_shape()
		shape.shape_type = shapes_2d.ShapeType.RECTANGLE
		shape.size = Vector2(size.x * 32, size.y * 32)
		shape.position = Vector2(pos.x * 32, pos.y * 32)
		
		match type:
			terrain_system.TERRAIN_TYPES.COVER:
				shape.color = Color(0, 0.7, 0, 0.5)
			terrain_system.TERRAIN_TYPES.HIGH_GROUND:
				shape.color = Color(0.7, 0.5, 0, 0.6)
			terrain_system.TERRAIN_TYPES.DIFFICULT:
				shape.color = Color(0.6, 0.3, 0, 0.4)
...
func _place_linear_obstacle(pos: Vector2i, length: int, horizontal: bool) -> void:
	for i in range(length):
		var obstacle_pos := Vector2i(
			pos.x + (i if horizontal else 0),
			pos.y + (0 if horizontal else i)
		)
