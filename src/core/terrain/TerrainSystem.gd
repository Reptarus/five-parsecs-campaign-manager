...
func initialize_grid(size: Vector2i) -> void:
	...
	for x in range(size.x):
		...
		for y in range(size.y):
			column.append(GlobalEnums.TerrainFeatureType.NONE)
		_terrain_grid.append(column)

func get_terrain_type(pos: Vector2i) -> int:
	if not _is_valid_position(pos):
		return GlobalEnums.TerrainFeatureType.NONE
...
func is_position_empty(pos: Vector2i) -> bool:
	if not _is_valid_position(pos):
		return false
	return _terrain_grid[pos.x][pos.y] == GlobalEnums.TerrainFeatureType.NONE
