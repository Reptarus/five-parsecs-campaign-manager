...
func add_character_entry(character: FiveParsecsCharacter) -> void:
	...
	stats_text += "Reactions: %d\n" % character.get_stat(GlobalEnums.CharacterStats.REACTIONS)
	stats_text += "Combat Skill: %d\n" % character.get_stat(GlobalEnums.CharacterStats.COMBAT_SKILL)
	stats_text += "Toughness: %d\n" % character.get_stat(GlobalEnums.CharacterStats.TOUGHNESS)
	stats_text += "Savvy: %d\n" % character.get_stat(GlobalEnums.CharacterStats.SAVVY)
	stats_text += "Tech: %d\n" % character.get_stat(GlobalEnums.CharacterStats.TECH)
	stats_text += "Navigation: %d\n" % character.get_stat(GlobalEnums.CharacterStats.NAVIGATION)
	stats_text += "Social: %d\n" % character.get_stat(GlobalEnums.CharacterStats.SOCIAL)
	add_text(stats_text)
	...
	for item in character.equipped_gear:
		if item.type != GlobalEnums.ItemType.MISC:
			gear_text += item.name + ", "
