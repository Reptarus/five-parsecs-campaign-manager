...
var parent: Variant = null # PathNode - can't type this properly in GDScript
...
func _init(battlefield: Node) -> void: 
	...
	if not is_instance_valid(self):
		return# Parameter validation - eliminates UNSAFE_CALL_ARGUMENT warnings
	if not is_instance_valid(self):
		return# Accept Node, will be BattlefieldManager
...
func _get_neighbors(node: Variant) -> Array:
	...
	for direction in _movement_directions:
		...
		if battlefield_manager and battlefield_manager.has_method("_is_valid_grid_position") and battlefield_manager._is_valid_grid_position(neighbor_pos):
			...
			if not TerrainTypes.blocks_movement(terrain_type):
				return [create_path_node(neighbor_pos)]
