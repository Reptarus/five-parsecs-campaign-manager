...
func update_state(new_values: Dictionary, skip_validation: bool = false) -> void:
	...
	state_changed.emit(old_state, current_state) # warning: return value discarded (intentional)
	state_updated.emit(current_state) # warning: return value discarded (intentional)
	is_transitioning = false
			...
			if result.has("valid") and result.has("valid") and not result.get("valid", true):
				...
				if result.has("issues"):
					...
					if result_issues is Array:
						...
						issues.append_array(rule_issues) # warning: return value discarded (intentional)
			
	state_validated.emit(is_valid, issues) # warning: return value discarded (intentional)
...
func _validate_state_types(state: Dictionary) -> Dictionary:
	...
	for key in state:
		...
		if not _is_valid_type(state_value):
			issues.append({"type": "type_mismatch", "key": key, "value": state_value}) # warning: return value discarded (intentional)
...
func _validate_required_fields(state: Dictionary) -> Dictionary:
	...
	for field in required_fields:
		if not state.has(field):
			issues.append({"type": "missing_field", "field": field}) # warning: return value discarded (intentional)
...
func _validate_value_ranges(state: Dictionary) -> Dictionary:
	...
	if state.has("credits"):
		...
		if credits_value is int:
			...
			if credits < 0 or credits > 1000000:
				issues.append({"type": "invalid_range", "field": "credits", "value": credits}) # warning: return value discarded (intentional)
	...
	if state.has("campaign_turns"):
		...
		if turns_value is int:
			...
			if turns < 0:
				issues.append({"type": "invalid_range", "field": "campaign_turns", "value": turns}) # warning: return value discarded (intentional)
			...
			if recovery_handlers.has(issue_type):
				...
				if handler_variant is Callable:
					...
					if handler.is_valid():
						...
						recovery_attempts.append(result) # warning: return value discarded (intentional)
						...
						recovery_attempts.append(failed_result) # warning: return value discarded (intentional)
						success = false
				else:
...
func _create_checkpoint(label: String = "") -> String:
	...
	state_checkpoint_created.emit(checkpoint_id) # warning: return value discarded (intentional)
...
func _restore_checkpoint(checkpoint_id: String) -> bool:
	...
	if checkpoint_data is Dictionary:
		...
		state_checkpoint_restored.emit(checkpoint_id) # warning: return value discarded (intentional)
		return true
...
func _add_to_history(state: Dictionary) -> void:
	...
	state_history.append(entry) # warning: return value discarded (intentional)
...
func reset_state() -> void:
	...
	_initialize_state()
	state_changed.emit(old_state, current_state) # warning: return value discarded (intentional)
	state_reset.emit() # warning: return value discarded (intentional)
