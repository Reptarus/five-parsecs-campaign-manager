...
func _validate_character() -> bool:
	...
	var name: String = _get_character_property(current_character, "character_name", "")
...
func _on_confirm_pressed() -> void:
	if _validate_character():
		if creator_mode == CreatorMode.CAPTAIN:
			character_created.emit(current_character) # warning: return value discarded (intentional)
		else:
			character_edited.emit(current_character) # warning: return value discarded (intentional)
		hide()

func _on_cancel_pressed() -> void:
	creation_cancelled.emit() # warning: return value discarded (intentional)
	hide()
