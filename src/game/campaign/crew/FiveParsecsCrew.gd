...
var ship_type: int = 0 # Will use FiveParsecsGlobalEnums.ShipType
var reputation: int = 0
var patrons: Array = []
var rivals: Array = []
var campaign_progress: int = 0
var campaign_type: int = 0 # Will use FiveParsecsGlobalEnums.CampaignType
...
func _init() -> void:
	super ()
	crew_name = "New Five Parsecs Crew"
	credits = 1500

func add_member(character: Variant) -> bool:
	# Parameter validation - eliminates UNSAFE_CALL_ARGUMENT warnings
	if not is_instance_valid(self):
		return false # Override to ensure we're adding FiveParsecsCrewMember instances
...
func generate_random_crew(size: int = 5) -> void:
	...
	for i: int in range(size):
		...
		# Randomize class
		member.character_class = randi() % FiveParsecsGlobalEnums.CharacterClass.size()
	...
	ship_type = randi() % GlobalEnums.ShipType.size()
