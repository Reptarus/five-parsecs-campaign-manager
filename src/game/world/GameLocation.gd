...
func add_mission(mission: Variant) -> void:
...
func remove_point_of_interest(poi_id: String) -> void:
	for i: int in range(points_of_interest.size()):
...
## This allows the EconomySystem to work with both location types
...
static func deserialize(data: Dictionary) -> GameLocation:
	...
	for i: int in range(trait_data.size()):
