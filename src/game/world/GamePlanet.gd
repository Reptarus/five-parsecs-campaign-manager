@tool
extends Resource
# This file should be referenced via preload instead of class_name
# to avoid conflicts with global script classes

const GamePlanetSelf = preload("res://src/game/world/GamePlanet.gd")

const GameDataManager = preload("res://src/core/managers/GameDataManager.gd")
# Keep these preloads until all files are updated to use class_name properly
...
func serialize() -> Dictionary:
	...
	for t in threats:
		threat_keys.append(GlobalEnums.ThreatType.keys()[t])
...
## Create a planet from serialized data
static func deserialize(data: Dictionary) -> Resource:
	var planet = GamePlanetSelf.new()
	...
	for threat in threats:
		if threat in GlobalEnums.ThreatType.keys():
			planet.threats.append(GlobalEnums.ThreatType[threat])
