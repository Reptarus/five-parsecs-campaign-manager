...
func _init() -> void:
	...
	if not Engine.is_editor_hint():
		...
		if _data_manager and _data_manager.has_method("ensure_data_loaded"):
			_data_manager.ensure_data_loaded()
		else:
...
func generate_world(num_sectors: int = 1) -> void:
	...
	# Generate _sectors
	for i: int in range(num_sectors):
...
func _generate_sector(sector_name: String, sector_position: Vector2) -> void:
	...
	# Generate planets
	for i: int in range(num_planets):
...
func _generate_planet(sector_name: String, sector_position: Vector2) -> GamePlanet:
	...
	if planet.planet_type == GlobalEnums.PlanetType.NONE:
		planet.planet_type = GlobalEnums.PlanetType.TEMPERATE
	...
	if planet.faction_type == GlobalEnums.FactionType.NONE:
		planet.faction_type = GlobalEnums.FactionType.NEUTRAL

	# Random environment
	planet.environment_type = randi() % GlobalEnums.PlanetEnvironment.size()
	if planet.environment_type == GlobalEnums.PlanetEnvironment.NONE:
		planet.environment_type = GlobalEnums.PlanetEnvironment.URBAN
	...
	for i: int in range(num_locations):
...
func _generate_location(planet: GamePlanet) -> GameLocation:
	...
	location.location_type = randi() % GlobalEnums.LocationType.size()
	if location.location_type == GlobalEnums.LocationType.NONE:
		location.location_type = GlobalEnums.LocationType.INDUSTRIAL_HUB
	...
	if randf() < 0.8:
		...
		location.faction_control = randi() % GlobalEnums.FactionType.size()
		if location.faction_control == GlobalEnums.FactionType.NONE:
			location.faction_control = GlobalEnums.FactionType.NEUTRAL
...
func _connect_planets_in_sector(planet_ids: Array) -> void:
	...
	while not (safe_call_method(unconnected_planets, "is_empty") == true):
		...
		safe_call_method(connected_planets, "append", [closest_unconnected]) # warning: return value discarded (intentional)
		unconnected_planets.erase(closest_unconnected)

	# Add some additional connections based on connection_density
	var num_additional_connections = int((safe_call_method(planet_ids, "size") as int) * connection_density)
	for i: int in range(num_additional_connections):
		var planet1_idx = randi() % (safe_call_method(planet_ids, "size") as int)
		var planet2_idx = randi() % (safe_call_method(planet_ids, "size") as int)
...
func _connect_locations_on_planet(planet: GamePlanet) -> void:
	...
	for location in planet.locations:
		safe_call_method(location_ids, "append", [location.location_id]) # warning: return value discarded (intentional)

	# Ensure all locations are connected in a minimum spanning tree
	if (safe_call_method(location_ids, "size") as int) <= 1:
		return

	var connected_locations = [location_ids[0]]
	var unconnected_locations = location_ids.slice(1)

	while not (safe_call_method(unconnected_locations, "is_empty") == true):
		var closest_distance = INF
		var closest_connected: String = ""
		var closest_unconnected: String = ""
		...
		safe_call_method(connected_locations, "append", [closest_unconnected]) # warning: return value discarded (intentional)
		unconnected_locations.erase(closest_unconnected)

	# Add some additional connections based on connection_density
	var num_additional_connections = int((safe_call_method(location_ids, "size") as int) * connection_density)
	for i: int in range(num_additional_connections):
		var loc1_idx = randi() % (safe_call_method(location_ids, "size") as int)
		var loc2_idx = randi() % (safe_call_method(location_ids, "size") as int)
...
func _connect_sectors() -> void:
	...
	if (safe_call_method(sector_names, "size") as int) <= 1:
		return

	for i: int in range((safe_call_method(sector_names, "size") as int) - 1):
...
func _add_traits_based_on_planet_type(planet: GamePlanet) -> void:
	...
	match planet.planet_type:
		GlobalEnums.PlanetType.DESERT:
			trait_id = "desert_world"
		GlobalEnums.PlanetType.ICE:
			trait_id = "ice_world"
		GlobalEnums.PlanetType.JUNGLE:
			trait_id = "jungle_world"
		GlobalEnums.PlanetType.TEMPERATE:
			trait_id = "frontier_world"
		GlobalEnums.PlanetType.ROCKY:
			trait_id = "mining_world"
		GlobalEnums.PlanetType.VOLCANIC:
			trait_id = "mining_world"
		_:
			trait_id = "frontier_world"
	...
	if randf() < 0.3:
		...
		if not (safe_call_method(traits, "is_empty") == true):
			second_trait = traits[randi() % (safe_call_method(traits, "size") as int)]
...
func _add_traits_based_on_location_type(location: GameLocation) -> void:
	...
	match location.location_type:
		GlobalEnums.LocationType.INDUSTRIAL_HUB:
			trait_id = "urban_world"
		GlobalEnums.LocationType.FRONTIER_WORLD:
			trait_id = "frontier_world"
		GlobalEnums.LocationType.TRADE_CENTER:
			trait_id = "urban_world"
		GlobalEnums.LocationType.MINING_COLONY:
			trait_id = "mining_world"
		_:
			trait_id = "frontier_world"
...
func _generate_planet_name() -> String:
	...
	if randf() < 0.3:
		node_name += prefixes[randi() % (safe_call_method(prefixes, "size") as int)] + " "

	# Always have a root name
	node_name += roots[randi() % (safe_call_method(roots, "size") as int)]

	# 40% chance to have a suffix
	if randf() < 0.4:
		node_name += " " + suffixes[randi() % (safe_call_method(suffixes, "size") as int)]
...
func _generate_location_name(planet_type: int) -> String:
	...
	var type_specific = {
		GlobalEnums.PlanetType.DESERT: ["Dune", "Oasis", "Mirage", "Dust", "Sand"],
		GlobalEnums.PlanetType.ICE: ["Frost", "Glacier", "Tundra", "Snowfield", "Iceberg"],
		GlobalEnums.PlanetType.JUNGLE: ["Grove", "Thicket", "Canopy", "Verdant", "Emerald"],
		GlobalEnums.PlanetType.TEMPERATE: ["Meadow", "Valley", "Hill", "Dale", "Field"],
		GlobalEnums.PlanetType.ROCKY: ["Ridge", "Peak", "Crag", "Stone", "Boulder"],
		GlobalEnums.PlanetType.VOLCANIC: ["Ember", "Ash", "Cinder", "Magma", "Forge"]
	}
	...
	if planet_type in type_specific and randf() < 0.6:
		...
		node_name += type_roots[randi() % (safe_call_method(type_roots, "size") as int)]
	else:
		node_name += roots[randi() % (safe_call_method(roots, "size") as int)]
...
func _generate_planet_description(planet_type: int) -> String:
	var descriptions = {
		GlobalEnums.PlanetType.DESERT: "A harsh, arid world with vast deserts and scarce water resources.",
		GlobalEnums.PlanetType.ICE: "A frozen wasteland with temperatures well below freezing most of the year.",
		GlobalEnums.PlanetType.JUNGLE: "A lush, overgrown world with dense vegetation and diverse wildlife.",
		GlobalEnums.PlanetType.TEMPERATE: "A world with moderate climate and varied ecosystems, suitable for human habitation.",
		GlobalEnums.PlanetType.ROCKY: "A rugged world with mountainous terrain and valuable mineral deposits.",
		GlobalEnums.PlanetType.VOLCANIC: "An unstable world with active volcanoes and geothermal activity.",
		GlobalEnums.PlanetType.OCEAN: "A world covered mostly by vast oceans with scattered island chains."
	}
...
func _generate_location_description(location_type: int) -> String:
	var descriptions = {
		GlobalEnums.LocationType.INDUSTRIAL_HUB: "A bustling center of manufacturing and production.",
		GlobalEnums.LocationType.FRONTIER_WORLD: "A remote outpost on the edge of settled space.",
		GlobalEnums.LocationType.TRADE_CENTER: "A busy marketplace where goods from across the sector change hands.",
		GlobalEnums.LocationType.PIRATE_HAVEN: "A lawless den of smugglers and outlaws.",
		GlobalEnums.LocationType.FREE_PORT: "An independent trading post free from faction control.",
		GlobalEnums.LocationType.CORPORATE_CONTROLLED: "A settlement under the strict control of a powerful corporation.",
		GlobalEnums.LocationType.TECH_CENTER: "A hub of research and technological innovation.",
		GlobalEnums.LocationType.MINING_COLONY: "A rugged settlement focused on extracting valuable resources.",
		GlobalEnums.LocationType.AGRICULTURAL_WORLD: "A peaceful community dedicated to food production."
	}
