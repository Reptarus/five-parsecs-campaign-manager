...
var HouseRulesController = preload("res://src/ui/components/combat/rules/house_rules_controller.tscn")
var controller: Node
...
func test_apply_combat_modifier() -> void:
	...
	# Note: We can't test the actual application since we don't have a real combat manager
	# Instead, we verify the effect was created correctly
	var effect = controller.rule_effects[rule_id]
...
func test_combat_state_changed() -> void:
	...
	# Note: We can't test the actual state change since we don't have a real combat manager
