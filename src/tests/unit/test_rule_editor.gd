...
var RuleEditor = preload("res://src/ui/components/combat/rules/rule_editor.tscn")
var editor: Node
...
func test_create_field_controls() -> void:
	...
	var fields = editor.fields_container.get_children()
	assert_eq(fields.size(), 3, "Should create all field controls")
	
	var value_control = editor.field_controls.get("value")
	...
	var condition_control = editor.field_controls.get("condition")
	...
	var target_control = editor.field_controls.get("target")
...
func test_get_rule_data() -> void:
	...
	editor._create_field_controls("combat_modifier")
	var value_control = editor.field_controls["value"]
	value_control.value = 2
	
	var data = editor.get_rule_data()
	...
	var value_field = data.fields.filter(func(f): return f.name == "value")[0]
...
func test_save_rule() -> void:
	...
	var params = get_signal_parameters(editor, "rule_saved")
	assert_eq(params[0], "", "Should emit empty ID for new rule")
	assert_eq(params[1].name, "Test Rule", "Should emit correct rule data")
...
func test_delete_rule() -> void:
	...
	var params = get_signal_parameters(editor, "rule_deleted")
	assert_eq(params[0], "test_id", "Should emit correct rule ID")
...
func test_preview_rule() -> void:
	...
	var params = get_signal_parameters(editor, "preview_requested")
	assert_eq(params[0].name, "Test Rule", "Should emit correct rule data")
...
func test_type_selection() -> void:
	...
	var combat_index = editor.type_option.get_item_index(1)
	...
	var resource_index = editor.type_option.get_item_index(2)
	...
	var state_index = editor.type_option.get_item_index(3)
