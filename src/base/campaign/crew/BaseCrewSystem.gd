...
func add_crew_member(member: Variant) -> bool:
	...
	safe_call_method(current_crew.crew_members, "append", [member])
	crew_changed.emit(current_crew)
	return true

func remove_crew_member(member: Variant) -> bool:
	# Parameter validation - eliminates UNSAFE_CALL_ARGUMENT warnings
	if not is_instance_valid(self):
		return false
	if member == null:
...
func set_captain(captain: Variant) -> void:
	# Parameter validation - eliminates UNSAFE_CALL_ARGUMENT warnings
	if not is_instance_valid(self):
		return
...
func add_connection(connection: Dictionary) -> bool:
	...
	safe_call_method(current_crew.connections, "append", [connection])
	crew_changed.emit(current_crew)
	return true

func remove_connection(connection_id: String) -> bool:
	for i: int in range(safe_call_method(current_crew.connections, "size") as int):
...
func set_ship(ship: Variant) -> void:
	# Parameter validation - eliminates UNSAFE_CALL_ARGUMENT warnings
	if not is_instance_valid(self):
		return
...
func get_crew_size() -> int:
	return safe_call_method(current_crew.crew_members, "size") as int + (1 if current_crew.captain != null else 0)
