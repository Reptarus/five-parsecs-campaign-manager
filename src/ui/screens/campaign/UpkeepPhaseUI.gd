...
func _calculate_ship_maintenance() -> int:
	...
	var base_cost := int(game_state.ship_hull_points / 10.0)
...
func _update_ship_display() -> void:
	if game_state.ship_hull_points:
		...
		repair_points_spin.max_value = min(game_state.ship_hull_points, game_state.credits / 2.0)
	else:
	...
	if selected_medical_crew:
		...
		if game_state.credits >= cost:
			...
			if turns >= 2: # Full recovery requires at least 2 turns of care
				selected_medical_crew.status = GlobalEnums.CharacterStatus.HEALTHY
			_update_crew_lists()
			_update_button_states()
