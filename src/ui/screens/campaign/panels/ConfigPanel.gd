...
func _handle_config_change() -> void:
	...
	panel_data_changed.emit(current_config)
...
func _throttle_config_change() -> void:
	...
	# Use a short delay to batch rapid changes (with null safety)
...
func get_data() -> Dictionary:
	...
	push_warning("ConfigPanel.get_data() is deprecated - use get_panel_data() instead")
...
func get_panel_data() -> Dictionary:
	"""Get panel data - standard interface implementation"""
	return get_config_data()


func _get_config_data_internal() -> Dictionary:
	"""Internal: Get configuration data. Use get_panel_data() as public API."""
	return get_config_data()
...
func _on_coordinator_set() -> void:
	"""Called when coordinator is assigned - sync initial config state"""
	print("ConfigPanel: Coordinator set, syncing initial state")
	...
	if coord and coord.has_method("get_unified_campaign_state"):
		...
		if state.has("campaign_config") and state.campaign_config is Dictionary:
			...
			if not config_data.is_empty():
				print("ConfigPanel: Found existing config in coordinator")
