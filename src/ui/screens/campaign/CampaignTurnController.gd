...
func _generate_terrain_setup_guide(mission_data: Dictionary) -> Dictionary:
	## Generate terrain setup suggestions from JSON data (battlefield_tables/terrain_types.json)
	## Falls back to generic suggestions if JSON loading fails
	...
	if dm and dm.has_method("load_json_file"):
		terrain_data = dm.load_json_file("res://data/battlefield_tables/terrain_types.json")

	# Roll or match a terrain type from JSON entries
	var selected_terrain: Dictionary = {}
	var entries: Array = terrain_data.get("entries", [])
	if not entries.is_empty():
		# Try to match by location keywords first
		var loc_lower: String = location.to_lower()
		for entry in entries:
			var tags: Array = entry.get("tags", [])
			var entry_type: String = entry.get("result", {}).get("type", "").to_lower()
			if loc_lower.find("urban") >= 0 and "urban" in tags:
				selected_terrain = entry.get("result", {})
				break
			elif loc_lower.find("wild") >= 0 and "natural" in tags:
				selected_terrain = entry.get("result", {})
				break
			elif (loc_lower.find("waste") >= 0 or loc_lower.find("desert") >= 0) and "wasteland" in tags:
				selected_terrain = entry.get("result", {})
				break
			elif (loc_lower.find("ship") >= 0 or loc_lower.find("station") >= 0) and entry_type == "ship_interior":
				selected_terrain = entry.get("result", {})
				break

		# Fallback: random roll from terrain table
		if selected_terrain.is_empty():
			var roll: int = randi() % 100 + 1
			for entry in entries:
				var roll_range: Array = entry.get("roll_range", [0, 0])
				if roll_range.size() >= 2 and roll >= roll_range[0] and roll <= roll_range[1]:
					selected_terrain = entry.get("result", {})
					break
	...
	if not selected_terrain.is_empty():
		...
		terrain["suggestions"] = [
			...
			"Add 1-2 line-of-sight blockers (buildings, large rocks)"
		]
