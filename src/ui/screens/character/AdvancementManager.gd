...
func _refresh_crew_list() -> void:
	...
	for character in crew_roster:
		var character_panel = _create_character_panel(character)
...
func _refresh_experience_display(character: Dictionary) -> void:
	...
	var available_advancements = xp / 5
...
func _refresh_injuries_display(character: Dictionary) -> void:
	...
	if injuries.is_empty():
		...
		for injury in injuries:
			var injury_panel = _create_injury_panel(injury)
...
func _refresh_advancement_options(character: Dictionary) -> void:
	...
	if available_advancements == 0:
		var no_advancement_label = Label.new()
	...
	for option in advancement_options:
		var option_panel = _create_advancement_option_panel(option)
...
func _get_advancement_options(character: Dictionary) -> Array[Dictionary]:
	...
	options.append({"name": "Improve Reactions", "type": "stat", "stat": "reactions", "cost": 1}) # warning: return value discarded (intentional)
	options.append({"name": "Improve Speed", "type": "stat", "stat": "speed", "cost": 1}) # warning: return value discarded (intentional)
	options.append({"name": "Improve Combat Skill", "type": "stat", "stat": "combat_skill", "cost": 1}) # warning: return value discarded (intentional)
	options.append({"name": "Improve Toughness", "type": "stat", "stat": "toughness", "cost": 1}) # warning: return value discarded (intentional)
	options.append({"name": "Improve Savvy", "type": "stat", "stat": "savvy", "cost": 1}) # warning: return value discarded (intentional)
	
	# Special abilities based on class
	match character.get("class", ""):
		"Soldier":
			options.append({"name": "Combat Veteran", "type": "ability", "description": "Bonus in combat", "cost": 1}) # warning: return value discarded (intentional)
		"Scientist":
			options.append({"name": "Tech Expert", "type": "ability", "description": "Bonus with gadgets", "cost": 1}) # warning: return value discarded (intentional)
		"Military":
			options.append({"name": "Tactical Mind", "type": "ability", "description": "Bonus to initiative", "cost": 1}) # warning: return value discarded (intentional)
...
func _on_heal_injury(injury: String) -> void:
	...
	injury_healed.emit(selected_character, {"name": injury}) # warning: return value discarded (intentional)
...
func _on_back_pressed() -> void:
	...
	if scene_router and scene_router.has_method("navigate_back"):
...
func _on_apply_advancement_pressed() -> void:
	...
	advancements.append(selected_advancement.name) # warning: return value discarded (intentional)
	...
	character_advanced.emit(selected_character, selected_advancement) # warning: return value discarded (intentional)
