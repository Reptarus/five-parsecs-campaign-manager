...
func _update_status_display(character: Character) -> void:
	...
	var status_color: Color = UIColors.COLOR_EMERALD

	# Check health status
	if character.health <= 0:
		status_text = "Unconscious"
		status_color = UIColors.COLOR_RED
		...
		if health_percent <= 0.25:
			...
			status_color = UIColors.COLOR_AMBER
...
func _update_portrait_display(character: Character) -> void:
	...
	if FileAccess.file_exists(default_portrait_path):
		...
		var bg_color: Color = UIColors.COLOR_TEXT_SECONDARY
		...
		match character.character_class:
			...
			_:
				bg_color = UIColors.COLOR_TEXT_SECONDARY
...
func _clear_display() -> void:
	...
	if status_value:
		...
		status_value.modulate = UIColors.COLOR_TEXT_SECONDARY

	if portrait:
		portrait.modulate = UIColors.COLOR_TEXT_SECONDARY
...
func highlight_character(highlight: bool) -> void:
	...
	if highlight:
		...
		tween.set_loops(3)
		tween.tween_property(self, "modulate", UIColors.COLOR_AMBER, 0.3)
