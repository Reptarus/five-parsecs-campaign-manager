...
func _setup_dice_system() -> void:
	# Get dice manager from autoload
...
func _create_test_button(text: String, callback: Callable) -> void:
...
func _on_manual_roll_completed(dice_roll: FPCM_DiceSystem.DiceRoll) -> void:
	_update_results("Manual Roll Completed: %s" % dice_roll.get_simple_text())

func _on_auto_roll_toggled(button_pressed: bool) -> void:
	dice_manager.set_auto_mode(button_pressed)
	_update_results("Auto Roll: %s" % ("Enabled" if button_pressed else "Disabled"))

func _on_show_animations_toggled(button_pressed: bool) -> void:
...
func demonstrate_integration() -> void:
