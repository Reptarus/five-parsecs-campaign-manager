...
func _validate_character_for_equipment(character: Character) -> bool:
	...
	# Validate background enum (Character stores these as String key names)
	var background = character.background if character.background != null else "MILITARY_BRAT"
	if not _is_valid_background_enum(background):
		push_warning("EquipmentGenerationScene: Invalid background for %s, defaulting to MILITARY_BRAT" % character.character_name)
...
func _is_valid_background_enum(background: Variant) -> bool:
	## Validate background enum value (accepts String key name or int)
	if background is String:
		return GlobalEnums.Background.has(background)
	if background is int:
		return background >= 0 and background < GlobalEnums.Background.size()
	return false

func _is_valid_class_enum(char_class: Variant) -> bool:
	## Validate character class enum value (accepts String key name or int)
	if char_class is String:
		return GlobalEnums.CharacterClass.has(char_class)
...
func _generate_fallback_equipment(character: Character) -> Dictionary:
	...
	# Basic weapon based on character class (Character stores class as String key name)
	...
	for character in crew_members:
		if character and is_instance_valid(character):
			var display_name = "%s (%s)" % [
				...
				character.character_class.replace("_", " ").capitalize()
			]
...
func _update_generation_ui_state() -> void:
	...
	if status_label:
		if generation_in_progress:
			...
			status_label.modulate = UIColors.COLOR_EMERALD
		else:
			status_label.text = "Equipment Generation Ready"
			status_label.modulate = Color.WHITE
