...
func _create_missing_auto_save_container() -> void:
	...
	var auto_save_container: Container = HBoxContainer.new()
...
func _create_missing_auto_save_toggle() -> void:
	...
	var container: Container = get_node("Panel/VBoxContainer/AutoSaveContainer")
	@warning_ignore("untyped_declaration")
	...
	# Note: This component uses direct initialization pattern (_initialize_component_direct)
	# The complex validation approach is not used here
	print("SaveLoadUI: Required nodes and signals defined (", required_nodes.size(), " nodes, ", signal_connections.size(), " connections)")

# Add this method to store node references (simplified version)
@warning_ignore("unused_parameter")
func _store_node_references(nodes: Dictionary) -> void:
	# This method is kept for compatibility but nodes are already set via @onready
	# The direct initialization pattern handles node setup differently
	print("SaveLoadUI: Node references already established via @onready pattern")
...
func _create_minimal_ui_fallback() -> void:
	...
	if not panel.has_node("VBoxContainer"):
		@warning_ignore("confusable_local_declaration", "untyped_declaration")
...
func _refresh_save_list() -> void:
	...
	var save_names = save_manager.get_save_list()

	@warning_ignore("untyped_declaration")
	for save_name in save_names:
		# Defensive check to ensure save_name is a string
		if not typeof(save_name) == TYPE_STRING:
			push_warning("SaveLoadUI: Unexpected type in save list: %s" % typeof(save_name))
			continue

		# Fetch the full save data dictionary using the save name
		@warning_ignore("unsafe_method_access", "untyped_declaration")
		var save_data: Dictionary = save_manager.get_save_details(save_name)

		# Ensure we got valid data before proceeding
		if save_data.is_empty():
			push_warning("SaveLoadUI: Could not retrieve details for save: %s" % save_name)
			continue

		@warning_ignore("unsafe_call_argument", "untyped_declaration")
		var display_text = _format_save_display(save_data)
		@warning_ignore("unsafe_call_argument", "untyped_declaration")
		var icon = _get_save_icon(save_data)
		@warning_ignore("unsafe_call_argument", "return_value_discarded")
		save_list.add_item(display_text, icon)
		@warning_ignore("unsafe_call_argument")
		save_list.set_item_tooltip(save_list.get_item_count() - 1, _format_save_tooltip(save_data))
		save_list.set_item_metadata(save_list.get_item_count() - 1, save_data)

func _format_save_display(save: Dictionary) -> String:
	var type_prefix: String = ""
	var save_name: String = save.get("name", "")
	if save_name.begins_with("autosave_"):
		type_prefix = "[Auto] "
	elif save_name.begins_with("quicksave_"):
		type_prefix = "[Quick] "

	return "%s%s (Turn %d) - %s" % [
		type_prefix,
		save_name.replace("autosave_", "").replace("quicksave_", ""),
		save.get("campaign_turn", 0),
		save.get("date", "Unknown Date")
	]
...
func _format_save_tooltip(save: Dictionary) -> String:
	...
	""" % [
		save.get("name", "N/A"),
		save.get("date", "N/A"),
		save.get("version", "N/A"),
		save.get("save_version", "N/A"),
		save.get("campaign_turn", 0),
		save.get("credits", 0),
...
func _get_save_icon(save: Dictionary) -> Texture2D:
	...
	# Use preload for icon resources when possible, fallback to load for dynamic paths
	@warning_ignore("untyped_declaration")
...
func _on_auto_save_toggled(enabled: bool) -> void:
	# Use proper settings manager for autosave configuration
	# Note: GameSettings is a Resource class, should be accessed through a manager
	# For now, we'll store the preference locally until proper settings management is implemented
	@warning_ignore("untyped_declaration")
	var settings_instance = GameSettings.new()
	settings_instance.set_auto_save_enabled(enabled)
	_show_status("Auto-save " + ("enabled" if enabled else "disabled"))
	# TODO: Integrate with proper settings persistence system
...
func _on_export_pressed() -> void:
	...
	@warning_ignore("unsafe_call_argument", "untyped_declaration", "unused_variable")
	var save_data = save_list.get_item_metadata(selected[0])
...
func _on_load_pressed() -> void:
	var selected: Array[int] = save_list.get_selected_items()
	if selected.is_empty():
		_show_status("Please select a save file", true)
		return
