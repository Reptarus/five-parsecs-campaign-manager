...
func _place_cover_feature(x: int, y: int) -> void:
	...
	match cover_pattern:
		1, 2, 3: # Straight line (horizontal)
			for i in range(3):
				if x + i < grid_size.x:
					positions.append(Vector2i(x + i, y))  # warning: return value discarded (intentional)
		4, 5: # Straight line (vertical)
			for i in range(3):
				if y + i < grid_size.y:
					positions.append(Vector2i(x, y + i))  # warning: return value discarded (intentional)
		6: # L-shape
			positions.append(Vector2i(x, y))  # warning: return value discarded (intentional)
			positions.append(Vector2i(x + 1, y))  # warning: return value discarded (intentional)
			positions.append(Vector2i(x, y + 1))  # warning: return value discarded (intentional)
...
func _setup_deployment_zones() -> void:
	...
	for x in range(grid_size.x - 4, grid_size.x):
		for y in range(grid_size.y):
...
func initialize_battle(crew_members: Array[Resource], enemies: Array[Resource], mission_data: Resource = null) -> void:
	...
	for crew_member in crew_members:
		...
		unit.team = "crew"
		crew_units.append(unit)  # warning: return value discarded (intentional)
		all_units.append(unit)  # warning: return value discarded (intentional)
	...
	for enemy in enemies:
		...
		unit.team = "enemy"
		enemy_units.append(unit)  # warning: return value discarded (intentional)
		all_units.append(unit)  # warning: return value discarded (intentional)
...
func _on_auto_deploy_clicked() -> void:
	...
	for i in range(min(crew_units.size(), crew_positions.size())):
...
func _auto_deploy_enemies() -> void:
	...
	var enemy_positions = deployment_zones["enemies"].duplicate()
...
class TacticalUnit:
	...
	func initialize_from_crew_member(crew_member: Resource) -> void:
		...
		name = crew_member.get("character_name") if crew_member.has("character_name") else "Crew Member"
		
		# Extract stats if available
		combat_skill = crew_member.get("combat_skill") if crew_member.has("combat_skill") else 0
		toughness = crew_member.get("toughness") if crew_member.has("toughness") else 0
		savvy = crew_member.get("savvy") if crew_member.has("savvy") else 0
		reactions = crew_member.get("reactions") if crew_member.has("reactions") else 0
	...
	func initialize_from_enemy(enemy: Resource) -> void:
		...
		name = enemy.get("name") if enemy.has("name") else "Enemy"
		
		# Extract enemy stats
		combat_skill = enemy.get("combat_skill") if enemy.has("combat_skill") else 0
		toughness = enemy.get("toughness") if enemy.has("toughness") else 0
		reactions = enemy.get("reactions") if enemy.has("reactions") else 0
