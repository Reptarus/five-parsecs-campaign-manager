...
func is_equipment_assigned() -> bool:
	...
	# If user explicitly marked complete, respect that decision
	if assignment_completed:
		return true

	# Check if any crew member has equipment via EquipmentManager
	var equipment_manager = get_node_or_null("/root/EquipmentManager")
	if equipment_manager and equipment_manager.has_method("get_character_equipment"):
		for crew_member in crew_data:
			var char_id: String = ""
			if crew_member is Dictionary:
				char_id = crew_member.get("character_id", crew_member.get("id", ""))
			elif "character_id" in crew_member:
				char_id = crew_member.character_id
			if not char_id.is_empty():
				var equipment: Array = equipment_manager.get_character_equipment(char_id)
				if equipment.size() > 0:
					return true

	# No equipment found - return false (will prompt user to confirm proceeding without)
	return false
