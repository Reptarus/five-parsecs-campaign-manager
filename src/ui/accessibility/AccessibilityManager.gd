...
func announce_to_screen_reader(text: String, priority: String = "normal") -> void:
	...
	# Process remaining queue on next frame (RefCounted can't own Timer nodes)
	if not announcement_queue.is_empty():
		_process_announcement_queue.call_deferred()
...
func _add_focus_indicator(element: Control) -> void:
	## Add visual focus indicator to element via theme overrides
	# Create a focus StyleBox with a visible border
	var focus_style := StyleBoxFlat.new()
	focus_style.bg_color = Color(0.0, 0.0, 0.0, 0.0) # Transparent background
	focus_style.border_color = UIColors.COLOR_FOCUS
	...
	element.add_theme_stylebox_override("focus", focus_style)
