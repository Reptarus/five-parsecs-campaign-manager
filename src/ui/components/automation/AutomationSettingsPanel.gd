...
• Simplified interface options"""
	...
	accessibility_desc_label.add_theme_color_override("font_color", UIColors.COLOR_TEXT_SECONDARY)
...
func _create_automation_section(parent: Node) -> void:
	...
	automation_description.add_theme_color_override("font_color", UIColors.COLOR_TEXT_SECONDARY)
...
func _create_dice_section(parent: Node) -> void:
	...
	dice_note.add_theme_color_override("font_color", UIColors.COLOR_TEXT_SECONDARY)
...
func _create_rules_section(parent: Node) -> void:
	...
	rules_note.add_theme_color_override("font_color", UIColors.COLOR_TEXT_SECONDARY)
...
func _show_all_options() -> void:
	# Accessibility mode: force helpful defaults on, show descriptions
	if show_calculations_check:
		show_calculations_check.button_pressed = true
		show_calculations_check.visible = true
	if always_confirm_check:
		always_confirm_check.button_pressed = true
		always_confirm_check.visible = true
	if rule_suggestions_check:
		rule_suggestions_check.button_pressed = true
		rule_suggestions_check.visible = true
	if accessibility_desc_label:
		accessibility_desc_label.visible = true

func _show_standard_options() -> void:
	# Standard mode: show all controls, hide verbose descriptions
	if show_calculations_check:
		show_calculations_check.visible = true
	if always_confirm_check:
		always_confirm_check.visible = true
	if rule_suggestions_check:
		rule_suggestions_check.visible = true
	if accessibility_desc_label:
		accessibility_desc_label.visible = false
