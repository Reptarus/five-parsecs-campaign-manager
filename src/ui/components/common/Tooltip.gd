...
func _update_arrow_shape() -> void:
	...
	arrow_points.append(Vector2(0, 0)) # warning: return value discarded (intentional)

	arrow_points.append(Vector2(arrow_size, -arrow_size)) # warning: return value discarded (intentional)
...
func _position_tooltip() -> void:
	...
	match final_position:
		Position.TOP:
			pos = Vector2(
				target_rect.position.x + target_rect.size.x / 2 - tooltip_size.x / 2,
		...
		Position.LEFT:
			pos = Vector2(
				...
				target_rect.position.y + target_rect.size.y / 2 - tooltip_size.y / 2
			)
		Position.RIGHT:
			pos = Vector2(
...
func _update_arrow_position(pos: Position, target_rect: Rect2) -> void:
	...
	match pos:
		Position.TOP:
			arrow_offset = Vector2(background.size.x / 2, background.size.y)
			rotation_deg = 180
		Position.BOTTOM:
			arrow_offset = Vector2(background.size.x / 2, 0)
			rotation_deg = 0
		Position.LEFT:
			arrow_offset = Vector2(background.size.x, background.size.y / 2)
			rotation_deg = 90
		Position.RIGHT:
			arrow_offset = Vector2(0, background.size.y / 2)
			rotation_deg = -90
		_:
			arrow_offset = Vector2(background.size.x / 2, 0)
			rotation_deg = 0
