...
func update_display() -> void:
	...
	if status.get("is_active", false):
		...
		status_label.text = "Story track inactive"
		status_label.modulate = UIColors.COLOR_TEXT_SECONDARY
...
func _on_story_clock_advanced(ticks_remaining: int) -> void:
	...
	var _unused1 := tween.tween_property(story_clock_label, "modulate", UIColors.COLOR_AMBER, 0.3)
...
func _on_evidence_discovered(evidence_count: int) -> void:
	...
	var tween: Tween = create_tween()
	var _unused1 := tween.tween_property(evidence_label, "modulate", UIColors.COLOR_EMERALD, 0.3)
