...
var type_colors := {
	...
	"difficult": UIColors.COLOR_AMBER,
	"special": UIColors.COLOR_PURPLE
}

var priority_colors := {
	1: UIColors.COLOR_RED, # Required
	2: UIColors.COLOR_AMBER, # Recommended
	3: UIColors.COLOR_AMBER # Optional
}
...
func _update_effects_display() -> void:
	...
	if suggestion_data.game_effects.size() > 0:
		...
		effects_header.add_theme_color_override("font_color", UIColors.COLOR_CYAN)
...
func _update_models_info() -> void:
	...
	if models_label and suggestion_data:
		if suggestion_data.suggested_models.size() > 0:
			...
			models_label.modulate = UIColors.COLOR_AMBER
		else:
			models_label.text = ""
...
func _update_styling_for_type() -> void:
	...
	if style is StyleBoxFlat:
		var type_color: Color = type_colors.get(String(suggestion_data.terrain_type), UIColors.COLOR_CYAN)
...
func _update_confirmation_state() -> void:
	...
	if confirm_button:
		...
		confirm_button.modulate = UIColors.COLOR_EMERALD
...
func _update_rejection_state() -> void:
	...
	if skip_button:
		...
		skip_button.disabled = true
		skip_button.modulate = UIColors.COLOR_RED
...
func _update_modification_state() -> void:
	...
	if modify_button:
		...
		modify_button.modulate = UIColors.COLOR_AMBER
...
func _create_help_dialog() -> AcceptDialog:
	...
	reference_label.modulate = UIColors.COLOR_TEXT_SECONDARY
