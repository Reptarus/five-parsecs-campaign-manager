...
func _unhandled_input(event: Variant) -> void:
...
func _handle_touch(event: Variant) -> void:
	...
	if event.pressed:
		...
		if (safe_call_method(touch_points, "size") as int) == 1:
		...
		if (safe_call_method(touch_points, "is_empty") == true):
...
func _handle_drag(_event: InputEventScreenDrag) -> void:
	if touch_points.has(_event.index):
		...
		if (safe_call_method(touch_points, "size") as int) == 2:
			_handle_pinch()
		else:
			long_press_timer.stop()
