...
func _create_crew_entry(name: String, data: Dictionary) -> Control:
	...
	if data.current == 0:
		dice_label.add_theme_color_override("font_color", UIColors.COLOR_RED)
	elif data.current < data.max:
		dice_label.add_theme_color_override("font_color", UIColors.COLOR_AMBER)
	else:
		dice_label.add_theme_color_override("font_color", UIColors.COLOR_EMERALD)
