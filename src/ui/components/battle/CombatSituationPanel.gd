...
func _build_toggles() -> void:
	...
	for key in modifiers:
		...
		if mod.value > 0:
			check.add_theme_color_override("font_color", Color(0.6, 1.0, 0.6))
		else:
...
func _update_display() -> void:
	...
	if hit_requirement_label:
		...
		if requirement <= 3:
			hit_requirement_label.add_theme_color_override("font_color", Color.GREEN)
		elif requirement >= 6:
			hit_requirement_label.add_theme_color_override("font_color", Color.RED)
		else:
			hit_requirement_label.add_theme_color_override("font_color", Color.YELLOW)
