...
func _update_phase_display(phase: int) -> void:
	current_phase = phase
	phase_label.text = GlobalEnums.PHASE_NAMES.get(phase, "Unknown")
	description_label.text = GlobalEnums.PHASE_DESCRIPTIONS.get(phase, "")
...
func _get_required_actions(phase: int) -> Array:
	match phase:
...
func _update_available_actions(phase: int) -> void:
...
func _on_phase_completed(_phase: int = 0, _completion_data: Dictionary = {}) -> void:
