...
func _create_health_pip(pip_index: int) -> Button:
	...
	pip.modulate = Color.RED if is_filled else Color.DARK_GRAY
...
func _update_activation_status() -> void:
	...
	activation_button.modulate = Color.GREEN if unit_data.activated_this_round else Color.WHITE
...
func highlight_health_change(old_health: int, new_health: int) -> void:
	...
	if new_health < old_health:
		flash_card(Color.RED, 0.3)
	elif new_health > old_health:
		flash_card(Color.GREEN, 0.3)
