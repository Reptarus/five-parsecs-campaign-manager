...
func _get_grid_size() -> Vector2:
	if terrain_system and not terrain_system.terrain_grid.is_empty():
		return Vector2(
			terrain_system.terrain_grid.size(),
...
func _update_particles(position: Vector2, effect_type: TerrainEffects.EffectType, active: bool) -> void:
	...
	if active:
		particles.position = position * cell_size + cell_size / 2.0
		particles.emission_rect_extents = cell_size / 2.0
