...
func _get_enum_string(value, enum_name: String) -> String:
	## Convert a potentially-int enum value to a display string
	if value is String:
...
func _get_reaction() -> int:
	## Get reaction stat - handles both 'reaction' and 'reactions' property names
	if not character_data:
		return 0
	if "reaction" in character_data:
		return character_data.reaction
	if "reactions" in character_data:
		return character_data.reactions
	return 0
...
func _create_portrait(size: int) -> TextureRect:
	## Create portrait with class-based icon from IconRegistry
	...
	# Icon overlay (set in _update_portrait based on character data)
	var icon_rect := TextureRect.new()
	icon_rect.name = "PortraitIcon"
	var icon_size := int(size * 0.7)
	icon_rect.custom_minimum_size = Vector2(icon_size, icon_size)
	icon_rect.position = Vector2(int(size * 0.15), int(size * 0.15))
	icon_rect.expand_mode = TextureRect.EXPAND_FIT_WIDTH_PROPORTIONAL
	icon_rect.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	icon_rect.modulate = COLOR_TEXT_PRIMARY
	clip.add_child(icon_rect)
...
func _update_display() -> void:
	...
	if _subtitle_label:
		var class_text = _get_enum_string(character_data.character_class, "CharacterClass")
		var bg_text = _get_enum_string(character_data.background, "Background")
...
const STAT_ICON_KEYS := {
	"REA": "reaction", "SPD": "speed", "CBT": "combat",
	"TGH": "toughness", "SAV": "savvy", "LCK": "luck"
}
...
func _create_stat_box(label_text: String, value_text: String, accent_color: Color) -> PanelContainer:
	...
	if not icon_key.is_empty():
		...
		if stat_tex:
			...
			icon.modulate = COLOR_TEXT_MUTED
