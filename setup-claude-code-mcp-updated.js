...
exports.ClaudeCodeMCPManager = void 0;
...
class ClaudeCodeMCPManager {
        ...
        switch (this.platform) {
            ...
            default:
                return {
                    ...
                    claude_desktop_config: path.join(homeDir, '.config', 'claude', 'claude_desktop_config.json'),
                    cursor_config: path.join(projectRoot, '.claude', 'settings.local.json'),
                    project_root: projectRoot,
                    python_executable: 'python3',
                    node_executable: 'node'
    ...
    async validateEnvironment() {
        ...
        if (!process.env.GOOGLE_API_KEY) {
            issues.push('GOOGLE_API_KEY environment variable not set');
        }
    ...
    async updateCursorConfig() {
        try {
            ...
            await fs.writeFile(this.paths.cursor_config, JSON.stringify(cursorConfig, null, 2));
    ...
    async syncFromDesktop() {
        try {
            ...
            for (const [serverName, serverConfig] of Object.entries(desktopConfig.mcpServers || {})) {
                claudeCodeConfig.mcp.servers[serverName] = this.adaptServerConfigForClaudeCode(serverConfig);
            }
            ...
            await fs.writeFile(this.paths.claude_code_config, JSON.stringify(claudeCodeConfig, null, 2));
     ...
     */
    adaptServerConfigForClaudeCode(desktopServerConfig) {
        const adapted = {
...
exports.ClaudeCodeMCPManager = ClaudeCodeMCPManager;
