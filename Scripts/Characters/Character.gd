...
@export var luck: int = 0  # Add this line
...
func serialize() -> Dictionary:
	var data = {
		"name": self.name,
		"species": GlobalEnums.Species.find_key(self.species),
		"background": GlobalEnums.Background.find_key(self.background),
		"motivation": GlobalEnums.Motivation.find_key(self.motivation),
		"character_class": GlobalEnums.Class.find_key(self.character_class),
		"reactions": reactions,
		"speed": speed,
...
func apply_character_effects(character_data: CharacterCreationData):
	...
	var background_data = character_data.get_background_data(int(self.background))
	if background_data:
	...
	var motivation_data = character_data.get_motivation_data(int(self.motivation))
	if motivation_data:
		apply_effects(motivation_data.get("effects", {}))
   
	# Apply class effects
	var class_data = character_data.get_class_data(int(self.character_class))
	if class_data:
