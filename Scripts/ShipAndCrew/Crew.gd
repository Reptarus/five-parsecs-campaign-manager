...
func get_cover_type(character: Character) -> CoverType:
	...
	for adj_pos in adjacent_positions:
		if is_valid_position(adj_pos) and battlefield[adj_pos.x][adj_pos.y] is CoverPiece:
			cover_count += 1
...
func use_item(_character: Character) -> void:
	# TODO: Implement item usage logic
	pass

func snap_fire(_character: Character, _target: Character) -> void:
	# TODO: Implement snap fire logic
	pass

func are_enemies_within_range(character: Character, range_value: float) -> bool:
...
func are_enemies_in_open(_character: Character) -> bool:
	for enemy in current_battle.player_characters:
...
func find_cover_within_range(character: Character, range_value: float) -> Vector2i:
	var best_position = character.position
	...
	for x in range(GRID_SIZE.x):
		for y in range(GRID_SIZE.y):
			...
			if battlefield[x][y] == null:
				...
				if get_cover_type(temp_character) != CoverType.NONE:
					var distance = character.position.distance_to(pos)
					if distance <= range_value and distance > character.position.distance_to(best_position):
						best_position = pos
...
func dash(character: Character, target_position: Vector2i) -> void:
	...
	var dash_position = Vector2(character.position) + (direction * float(character.speed * 2))
...
class Battle:
	...
	func _init(_player_characters: Array[Character], _enemies: Array[Character]) -> void:
	...
	func are_all_players_defeated() -> bool:
		return player_characters.all(func(player: Character) -> bool: return player.is_defeated())
