...
| BUG-100 | Window never filled 4K — saved display mode applied only on Settings screen, never at boot | `project.godot window/size/mode=2` (Maximized first-run default) + `GameState._restore_window_state_at_boot()` in `_ready()` replays `user://window.ini`. AppOptions.gd is DEAD/conflicting — do NOT wire |
| BUG-101 | Terrain bled off-grid. **Premature-verified twice.** TRUE cause: `ScalableVectorShape2D` draws its body centered on `offset` in LOCAL space and `offset` is rotated by node rotation → drawn center = `position + offset.rotated(rot)`, NOT `position`. Clamp protected the wrong point | Back-solve `svs.position = clamped_center - offset.rotated(rot)` + `stroke_width/2` envelope. **Regression watch**: any clamp/placement on SVS must clamp the DRAWN center, not `position`. Verify via `get_bounding_rect()`×transform vs grid rect |
| BUG-102 | First-render top-left cluster. TRUE cause: `BattlefieldGridPanel._update_map_cell_size()` mutated `cell_size` 16/24→48 on resize AFTER placement baked | `cell_size` is the STABLE placement base (24), NEVER mutated; panel resize handler neutered. **Regression watch**: never reintroduce a `cell_size` setter; on-screen scale is the container transform |
| BUG-103/104/105 | Legend hardcoded 12 entries / Tools accordion 5 unlabeled collapsed / tooltip showed raw features incl. hidden scatter | Data-driven `_rebuild_legend()` from `get_rendered_legend_keys()`; accordion `subtitle`+`open_section(0)`+hint (also wired 2 unwired tools); tooltip+popover use `get_rendered_feature_labels()` single source |
| CLR-101 | (NOT A BUG) Objective marker "stuck dead center" — flagged as defect | Verified VERBATIM rules-correct vs Core Rules PDF p.90 (Access/Acquire/Secure/Deliver = "exact center of the table/battlefield"). Position kept (moving it violates data-integrity + makes app rules-incorrect); added verbatim p.90 rule cite to the marker. **Do not "fix" rules-correct behavior** |
| TEST-HARNESS | `test_post_battle_success_cascade.gd` errored: called `start_post_battle_phase()` on detached `PostBattlePhase.new()` → `_ensure_subsystems()` `get_node("/root/..")` from outside tree | Removed the vestigial orchestration call (assertion was orchestration-free). **Gotcha**: even `get_node_or_null("/root/X")` errors from a detached node — never call tree-dependent methods on bare `.new()` instances in unit tests; add to tree or don't call |
| INJURY-DESC | `src/core/services/InjurySystemService.gd:63` accessed `range_data.description` but `INJURY_ROLL_RANGES` entries are `{min, max}` only (no `description` key) → every post-battle injury roll logged `SCRIPT ERROR` + returned a blank description. Pre-existing since 2026-04-02 (commit `43537887`). NOT a hard player-facing crash — Godot continues past Dictionary missing-key access, `is_fatal`/`recovery_turns` stayed correct; debugger-break first seen via MCP `run_script` was a debug-mode artifact, not a normal-play halt | Fixed → `InjurySystemConstants.get_injury_description(injury_type)` (canonical `INJURY_DESCRIPTIONS`, mirrors adjacent `type_name` lookup, no fabricated data). **Regression watch**: `test_injury_determination` 13/13 passed both before AND after the fix — the test's data path didn't trip the SCRIPT-ERROR line. Empirical d100 sweep is the gate here, not the unit suite. **Verification rule**: when a function is on a critical user path, sweep its full input domain via MCP `run_script`, don't trust the suite alone |
