...
- **Responsive / adaptive layout, rotation, mobile/tablet, breakpoints, portrait** → Read `docs/sop/responsive-adaptive-ui.md` (ResponsiveManager DPI-aware breakpoints + `layout_class_changed` rotation signal + `get_effective_columns()` + the base-class convergence + gotchas). `panel-patterns.md` covers only the legacy per-panel basics.
- **Multi-pane screen (N side-by-side panels that must collapse on small/portrait)** → Use `AdaptivePanelGroup` (`src/ui/components/base/AdaptivePanelGroup.gd`): `add_pane(control, title)`, `portrait_mode = STACK|TABS`. Panes reparent in ONCE, never per-rotation. See the SOP.
- **"Why doesn't my screen collapse/rotate on a phone/tablet?"** → It's almost always a width-bucket-keyed override (branch on `should_use_single_column()` not the bucket), a `_ready()` override missing `super._ready()`, or reading `get_visible_rect()` instead of `ResponsiveManager`. See the SOP gotchas.
...
| `src/ui/screens/campaign/CampaignScreenBase.gd` | `CampaignScreenBase` | Lightweight base for campaign screens (dashboard, etc.). Owns the responsive convergence: `_resolve_layout_mode`, `layout_class_changed` wiring, orientation-aware dedupe |
| `src/autoload/ResponsiveManager.gd` | Autoload | **Single source of truth** for breakpoints (DPI-aware) + orientation. `get_effective_columns()`, `should_collapse_to_single_column()`, `layout_class_changed` signal. See `docs/sop/responsive-adaptive-ui.md` |
| `src/ui/components/base/AdaptivePanelGroup.gd` | `AdaptivePanelGroup` | Reusable N-pane group: grid / stack / tabs by effective columns. No per-rotation reparenting |
| `src/ui/components/base/ResponsiveContainer.gd` | `ResponsiveContainer` | Local two-pane H↔V switch; AUTO consults ResponsiveManager |
...
25. **Responsive: use `ResponsiveManager`, never `get_visible_rect()` for breakpoints** (Jun 2026 mobile re-pivot). With `canvas_items`+`expand`+square 1080 base, portrait content is ALWAYS ~1080 wide, so the content rect can't tell a phone from a tablet. `ResponsiveManager` classifies by density-independent physical size (`window_get_size()/screen_get_scale()`). Call `get_effective_columns()` / `should_collapse_to_single_column()` (orientation-aware), NOT the legacy `get_optimal_columns()`. Full SOP: `docs/sop/responsive-adaptive-ui.md`.
26. **Responsive: connect `layout_class_changed`, not only `breakpoint_changed`** — a constant-width portrait↔landscape rotation does NOT change the width bucket, so `breakpoint_changed` never fires; `layout_class_changed` fires on bucket change OR rotation. Base classes (CampaignScreenBase / BaseCampaignPanel) already wire it; orientation-dependent `_apply_*_layout` overrides must branch on `should_use_single_column()` (NOT the width bucket) or they re-fire on rotation but change nothing. Subclasses overriding `_ready()` MUST call `super._ready()` or lose all wiring.
27. **Multi-pane screens → `AdaptivePanelGroup`, panes reparent in ONCE** — for N side-by-side panels that collapse to stack/tabs on small screens. Mode changes only toggle columns/visibility/TabBar; reparenting panes every rotation would churn `_enter_tree`/`_exit_tree`. `add_pane(control, title)` + `portrait_mode`. The "3-column screen → 1 column" case also needs an OUTER scroll + content-sized columns (don't nest inner+outer ScrollContainers).
