...
11. **Character transfer into Tactics is SHIPPED (Jun 4)** — The 4-mode `CharacterTransferService` canonical hub now interconnects all 4 modes any-to-any. A transferred character becomes a **named veteran** (an "officer or hero" figure, Tactics p.185) stored in a serialized `veteran_characters[]` array on TacticsCampaignCore via `add_veteran_character()` — NEVER a squad unit in `campaign_units[]`, so veterans stay OUT of points validation (the book uses "no points cost formula", p.184). Core methods: `add_veteran_character()` (applies a tagged playability floor of >=1 Kill Point), `remove_veteran_character()`, `get_veteran_characters()`. `CampaignScreenBase._add_character_to_mode()` "tactics" case dispatches to `add_veteran_character()` (was a push_warning placeholder). UI: TacticsDashboard "Commission Veteran" card (`TacticsVeteranImportPanel.gd`) + "Retire Veteran Out" 3-target overlay. **Data integrity DONE**: `convert_to_tactics` was verified against Tactics p.184 — the invented `military_backgrounds` list was removed (replaced with a "military"/"war-torn" substring check grounded in real gear_database.json backgrounds, because the book says only "+2 with a military-type background" with no enumerated list); the GAME_BALANCE_ESTIMATE tag is GONE. Tested: `tests/unit/test_tactics_transfer.gd` (9 tests; 24/24 total transfer tests pass). See `cross-mode-safety.md`
