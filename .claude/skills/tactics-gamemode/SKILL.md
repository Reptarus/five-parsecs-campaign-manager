...
11. **Character transfer into Tactics is P2 — NOT BUILT** — The 4-mode `CharacterTransferService` canonical hub exists, but the Tactics leg is a Phase 2 placeholder (`_add_character_to_mode` pushes a warning for `tactics`). Army lists stay species-profile-based; when P2 lands, a transferred character becomes a **named veteran attachment** in a NEW `veteran_characters[]` array on TacticsCampaignCore, NOT a squad unit in `campaign_units[]` (would break points validation). HARD PREREQUISITE: replace the invented `military_backgrounds` list in `convert_to_tactics` (GAME_BALANCE_ESTIMATE / UNVERIFIED) with the real Tactics p.184 table first. See `cross-mode-safety.md`
