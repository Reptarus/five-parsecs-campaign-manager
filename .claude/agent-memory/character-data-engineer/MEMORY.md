...
Status: ALL 4 modes SHIPPED (Bug Hunt ↔ 5PFH Foundation + Planetfall P1 + **Tactics P2, Jun 4**). Tests: `tests/unit/test_character_transfer_hub.gd`, `tests/unit/test_planetfall_transfer.gd`, `tests/unit/test_tactics_transfer.gd` (24/24 total transfer tests pass). Reward attachments are 5PFH-only (`target_mode == "five_parsecs"`).

**Tactics conversion is book-faithful (Jun 4)**: `convert_to_tactics` / `convert_from_tactics` were verified against Tactics PDF p.184 ("Converting Characters") and THREE fabrications were removed: (1) the invented `military_backgrounds` list → replaced with a "military"/"war-torn" substring check grounded in the real `gear_database.json` backgrounds (book says only "+2 with a military-type background", NO enumerated list); (2) a `max(luck,1)` KP floor → book is exactly "1 Kill Point per Luck point", so the playability floor moved to the veteran layer (tagged) and the conversion stays book-exact; (3) a "military property, equipment not transferred" strip → book says "carry weapons over as they are". Combat cap +2, Toughness cap 5, and "each Kill Point after the first becomes 1 Luck" on export were confirmed CORRECT. The `military_backgrounds` GAME_BALANCE_ESTIMATE tag is GONE — no longer a blocker. A transferred character becomes a NAMED VETERAN in `TacticsCampaignCore.veteran_characters[]` (new serialized array, never `campaign_units[]` — veterans stay out of points validation per p.184 "no points cost formula"). New core methods: `add_veteran_character()` / `remove_veteran_character()` / `get_veteran_characters()`.
