...
**Reward suppression**: 5PFH-specific exit rewards (Bug Hunt mustering credits / +1 Story Point / +Sector Government patron) attach ONLY when `target_mode == "five_parsecs"`. **Lossless snapshot**: each imported char embeds its canonical form under a `snapshot` key so a later muster-out restores the original verbatim (`export_to_canonical` short-circuits on the snapshot). ALL 4 modes now interconnect any-to-any: Tactics (P2) SHIPPED Jun 4 — a transferred char becomes a NAMED VETERAN in `TacticsCampaignCore.veteran_characters[]`. 24/24 gdUnit4 transfer tests pass (`tests/unit/test_tactics_transfer.gd` adds 9). P3 persistent barracks is DEFERRED.
