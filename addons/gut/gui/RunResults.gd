...
func set_font(font_name, size):
	# For Godot 4.x we use a different approach for fonts
	var font_path = 'res://addons/gut/fonts/' + font_name + '-Regular.ttf'
	...
	if ResourceLoader.exists(font_path):
		...
		if loaded_font != null:
			...
			# Apply the font to the tree control if it has a method to accept it
			if _ctrls.tree.has_method("set_font"):
				_ctrls.tree.set_font(_font, size)
