...
func _get_obj_filename(thing):
	...
	if (thing.get_script() == null):
		...
		# Add a try-catch block to safely handle inst_to_dict calls
		if thing is Resource and not thing.resource_path.is_empty():
			...
			# For objects that aren't proper resources, just use their class name
			var script = thing.get_script()
			if script:
				if script.resource_path:
					filename = _get_filename(script.resource_path)
				else:
					filename = script.get_instance_base_type()
			else:
				filename = thing.get_class()
