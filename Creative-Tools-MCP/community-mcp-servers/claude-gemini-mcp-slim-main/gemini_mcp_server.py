...
async def handle_file_requests():
    requests_dir = Path("/mnt/c/Users/elija/SynologyDrive/Godot/five-parsecs-campaign-manager/mcp_bridge/requests")
    responses_dir = Path("/mnt/c/Users/elija/SynologyDrive/Godot/five-parsecs-campaign-manager/mcp_bridge/responses")
    ...
    while True:
        for request_file in requests_dir.glob("*.json"):
            try:
                ...
                with open(responses_dir / f"{request_data['request_id']}.json", "w") as f:
                    json.dump(response, f)

                request_file.unlink()
            except Exception as e:
                logger.error(f"Error processing request {request_file.name}: {e}")
...
async def main():
    ...
    try:
        ...
        async with stdio_server() as (read_stream, write_stream):
            ...
            asyncio.create_task(handle_file_requests())
            await server.run(
                read_stream,
                write_stream,
