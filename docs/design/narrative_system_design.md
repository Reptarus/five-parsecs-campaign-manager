...
**Version**: 1.1 (Promoted from draft, SceneStage architectural revision)
**Original Date**: April 16, 2026
**Revised**: May 21, 2026 (SceneStage as single rendering path, foundation-only Phase 1 MVP, per-phase art_tag retrofit)
**Inspiration**: King of Dragon Pass / Six Ages
**Scope**: Core Rulebook campaign experience (9-phase turn flow)
**Build plan**: [`docs/plans/narrative-screen-phase-1.md`](../plans/narrative-screen-phase-1.md) — current authoritative build target

---

## 0. Locked design decisions (May 21, 2026)

These supersede earlier assumptions in this doc where they conflict. Plan-level details live in `docs/plans/narrative-screen-phase-1.md`.

1. **SceneStage is the only rendering path for illustrations.** The original design assumed `IllustrationLayer` = single flat `TextureRect`. With `src/ui/screens/narrative/SceneStage.gd` shipped and the PSD layer-extraction pipeline producing composable scenes, NarrativeScreen renders all illustration via SceneStage. Flat single-PNG art is a degenerate one-layer scene (single bg_layers entry, no actors). One mental model, one z-order, atmosphere overlays slot in naturally as a SceneStage sibling.
2. **Foundation-only Phase 1 MVP — no phase-panel integration in Phase 1.** Original Phase 1 wired into CharacterPhasePanel; revised Phase 1 ships the component + test harness only. Phase 2+ picks the first integration target (likely Story Track for its data readiness and event-count tightness).
3. **Per-phase art_tag retrofit, not bulk.** Original implementation phases implied art_tag fields added to all event JSONs early. Revised: each integration phase retrofits only its events. Phase 1 touches zero existing event JSONs.
4. **Quote pools scaffold at 1 quote per role-mood cell in Phase 1.** 18 quotes total proves the swapping system. Content expansion to ~180 happens during Phase 2-4 integration as advisor coverage matters.
5. **Atmosphere overlay (separate research arc) slots in as a SceneStage sibling.** See `docs/research/scene-stage-atmosphere.md`. Parked pending NarrativeScreen activation.
...
### Architecture (revised May 21, 2026 — SceneStage as single rendering path)
...
│
├── IllustrationFrame (Control, top 55% of screen)
│   └── SceneStage (Control, the only rendering path)
│       ├── For extracted PSDs: scene_id loads res://data/scenes/{scene_id}.json
│       │   (composable bg+actor+fx+prop stack, can toggle actors per event variant)
│       ├── For flat single-PNG art: degenerate manifest with one bg_layers entry
│       ├── Fallback: gradient atmosphere (world-theme-tinted ColorRect) when
│       │   neither scene_id nor flat art tag resolves to a real asset
│       └── (Future) Sibling SceneAtmosphereLayer for particle overlays
│           (parked research at docs/research/scene-stage-atmosphere.md)
│
...
## 8. Implementation Phases (revised May 21, 2026)

### Phase 1: Foundation — no integration target, no event JSONs touched

Authoritative plan: `docs/plans/narrative-screen-phase-1.md`. Summary:

- Create `NarrativeScreen.gd` (uses SceneStage as the only rendering path)
- Create `NarrativeTextGenerator.gd` (procedural openers from atmosphere_openers.json)
- Create `AdvisorSystem.gd` (crew→role matching + quote retrieval)
- Create `NarrativeChoiceButton.gd` (consequence-hint button)
- Create `narrative_demo.tscn` + `narrative_demo.gd` (test harness with mock events)
- Author scaffolding JSON: atmosphere_openers.json, advisor_quotes.json (1 per cell = 18 quotes), species_personality.json, mock_events.json
- Gradient/colored-initial fallback art (no real PNGs needed)
- **No phase panels modified. No event JSONs modified.** Component fully testable in isolation.

### Phase 2: First integration — Story Track (recommended)

Story Track is the tightest first integration: 7 events, existing `narrative_intro` + `narrative_briefing` JSON fields, opt-in firing (low blast radius).

- Retrofit `art_tag: "story_event_NN"` on the 7 events in `data/story_track_missions/`
- Wire `StoryPhasePanel` to call `NarrativeScreen.present()` for story events
- Expand quote pools as needed for story-event advisor coverage
- Validates the full integration shape with the lowest-risk event surface

### Phase 3: CharacterPhasePanel (30 D100 character events)

- Original Phase 1 target, now Phase 3 with NarrativeScreen substrate proven
- Retrofit `art_tag` + `advisor_role` on the 30 character events
- Quote pool expansion to ~3-5 per role-mood cell

### Phase 4: CrewTaskEventDialog replacement

- Bulk content work — 200+ trade/exploration entries retrofitted with art_tags
- Replace existing Window-based dialog with full-screen NarrativeScreen
- Heuristic art_tag pre-population script + manual review tool (mirror the PSD review HTML pattern)

### Phase 5: TravelPhase + PostBattle narrative beats

- D100 starship travel events (richest narrative events in the book)
- 3-5 PostBattle steps that benefit from narrative framing (others stay management)

### Phase 6: Polish

- Typewriter text effect (accessibility toggle)
- Ken Burns / parallax on illustrations
- Transition animations between management/narrative modes
- Sound design hooks (ambient audio per art_tag category)
- Atmosphere overlay activation (parked research)
- Steam screenshot optimization

### Art Drop-In (parallel track, any phase)
