...
> **STATUS NOTE (February 2026)**: This list was generated January 2025. Since then, the project
> underwent major restructuring: Godot 4.5.1→4.6 migration, enum refactoring, character class
> consolidation (Character.gd is now canonical, ~500+ lines), dead signal removal, dead file
> deletion (EnemyDeploymentManager.gd deleted), CampaignStateService autoload removed, and
> extensive tech debt clearance. Some items below may already be resolved. Verify before acting.
>
> Key cleanup already completed (Feb 2026):
> - `EnemyDeploymentManager.gd` deleted (9 empty stubs)
> - 3 dead signals removed from BaseCombatManager + BaseBattlefieldManager
> - `CampaignStateService` autoload removed (redundant with GameState)
> - Character class consolidated: `Character.gd` is canonical, `BaseCharacterResource` and
>   `character_base.gd` are thin redirects
> - `src/core/character/Character.gd` is NOT a compatibility stub anymore — it's the canonical 500+ line implementation
...
cd /mnt/c/Users/admin/SynologyDrive/Godot/five-parsecs-campaign-manager
