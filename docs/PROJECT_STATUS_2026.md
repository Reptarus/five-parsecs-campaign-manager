...
| Cross-Mode Character Transfer Framework (Jun 2026) | **Foundation + Planetfall P1 SHIPPED** — canonical-hub `CharacterTransferService` (5PFH/Bug Hunt/Planetfall/Tactics), 9 book-defined + 3 composed routes, reward-suppression (5PFH-only exit rewards), lossless snapshot, `user://transfers/` file-drop, mode-generic `CampaignScreenBase` pickup, `FiveParsecsCampaignCore.add_crew_member()` chokepoint, Planetfall import (Class Training D6) + muster-out, fixed previously-broken Bug Hunt muster-out pickup, `convert_from_planetfall` debt-swap data fix (pp.165-166). 15/15 gdUnit4 tests pass. P2 Tactics named-veteran NEXT (not built); P3 barracks deferred. See `docs/sop/cross-mode-transfer.md` |
...
- **Cross-Mode Character Transfer (Jun 2026, Foundation + Planetfall P1)**: canonical-hub `CharacterTransferService` interchange (full 5PFH Character dict), any-to-any via export-to-canonical + import-from-canonical (9 book-defined routes + 3 composed through the canonical), reward-suppression for 5PFH-only exit rewards, lossless `snapshot` round-trip, `user://transfers/` direct file-drop (no barracks), mode-generic pickup in `CampaignScreenBase`, `FiveParsecsCampaignCore.add_crew_member()` crew-addition chokepoint. Fixed the broken Bug Hunt muster-out pickup and corrected the `convert_from_planetfall` ending matrix (debt prepayment, not forgiveness). See `docs/sop/cross-mode-transfer.md`
