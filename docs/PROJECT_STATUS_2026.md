...
**Last Updated**: September 4, 2026 — **Sep 3-4 sprint complete and committed** (`c4317b993` on `campaign-editor-and-fixits`): the 8-phase battle-screen sprint, a Core-Rules page walk, and two real-hardware tablet deploys (#14 found 11 defects, #15 closed the last two — **both of which turned out to be misdiagnosed in #14**). Current gates: `verify_battle_ui` **137/0** (was 80), `verify_post_battle` 47/0, `verify_story_track` 9/0, 9 lints CLEAN with `lint_orphan_assets` `orphans=0`. **All 11 tablet fixes are now CLOSED ON HARDWARE** (deploy #16, Sep 4) — see [QA_STATUS_DASHBOARD.md](QA_STATUS_DASHBOARD.md), which is the maintained detail. The separate Aug 8-14 tablet sprint remains ⏸ PAUSED; read [qa/PICKUP_2026-08-14.md](qa/PICKUP_2026-08-14.md) before resuming that. The rules-wiring ledger remains CLOSED (0 open / 136 fixed / 1 corrected), with 18 further rows fixed by the Sep 3 page walk. `verify_rotation` is **23/0 PASS** and `verify_layout` **166/2** when run the way their docblocks require — WINDOWED (not `--headless`) and with `-- campaign=`. The long-standing RED was harness misuse; fixing it surfaced a real desktop bug (`user://window.ini` could persist `MODE_MINIMIZED`, so the app launched minimized and re-minimized itself when Settings opened). Everything below the milestone block dates to April 8 2026 (Session 52) and has NOT been re-verified; treat it as history.
...
| Cross-Mode Character Transfer Framework (Jun 2026) | **Foundation + Planetfall P1 + Tactics SHIPPED** — canonical-hub `CharacterTransferService` (5PFH/Bug Hunt/Planetfall/Tactics), all 4 modes interconnect any-to-any, 9 book-defined + 3 composed routes, reward-suppression (5PFH-only exit rewards), lossless snapshot, `user://transfers/` file-drop, mode-generic `CampaignScreenBase` pickup, `FiveParsecsCampaignCore.add_crew_member()` chokepoint, Planetfall import (Class Training D6) + muster-out, fixed previously-broken Bug Hunt muster-out pickup, `convert_from_planetfall` debt-swap data fix (pp.165-166). Tactics import = NAMED VETERAN in `TacticsCampaignCore.veteran_characters[]` (`add_veteran_character()`, never `campaign_units[]`); book-faithful conversion (p.184, fabricated `military_backgrounds` list removed); TacticsDashboard "Commission Veteran" + "Retire Veteran Out". 24/24 gdUnit4 transfer tests pass. P3 barracks deferred. See `docs/sop/cross-mode-transfer.md` |
...
- **Cross-Mode Character Transfer (Jun 2026, Foundation + Planetfall P1 + Tactics)**: canonical-hub `CharacterTransferService` interchange (full 5PFH Character dict), all 4 persistent modes interconnect any-to-any via export-to-canonical + import-from-canonical (9 book-defined routes + 3 composed through the canonical), reward-suppression for 5PFH-only exit rewards, lossless `snapshot` round-trip, `user://transfers/` direct file-drop (no barracks), mode-generic pickup in `CampaignScreenBase`, `FiveParsecsCampaignCore.add_crew_member()` crew-addition chokepoint. Fixed the broken Bug Hunt muster-out pickup and corrected the `convert_from_planetfall` ending matrix (debt prepayment, not forgiveness). Tactics import = NAMED VETERAN in `TacticsCampaignCore.veteran_characters[]` (`add_veteran_character()`, never `campaign_units[]`); book-faithful conversion (p.184/185, fabricated `military_backgrounds` list removed); TacticsDashboard "Commission Veteran" + "Retire Veteran Out". 24/24 gdUnit4 transfer tests pass. See `docs/sop/cross-mode-transfer.md`
