...
```
BattleScreen (Top Level)
    │
    ├─ BattleStateMachine (Logic)
    │   │
    │   ├─ Signals UP: battle_started, battle_ended, state_changed, phase_changed, round_started
    │   └─ Called DOWN: start_battle(), add_combatant(), advance_phase(), complete_unit_action()
    │
    └─ BattleHUDCoordinator (UI Display)
        │
        ├─ Signals UP: character_action_requested, character_damage_applied, enemy_casualty_registered
        │
        ├─ Called DOWN: initialize_with_battle_state(), update_character_health(), add_character_stun()
        │
        └─ Child Components:
            │
            ├─ CharacterStatusCard (per crew member)
            │   ├─ Signals UP: action_used, damage_taken, stun_marked, character_selected
            │   └─ Called DOWN: set_character_data(), apply_damage(), add_stun_marker(), reset_round()
            │
            ├─ ObjectiveDisplay
            │   ├─ Signals UP: objective_rolled, objective_acknowledged
            │   └─ Called DOWN: roll_objective(), display_objective()
            │
            └─ MoralePanicTracker
                ├─ Signals UP: morale_check_triggered, enemy_fled, panic_occurred
                └─ Called DOWN: set_enemy_count(), add_casualty(), roll_morale_check(), new_round()
```
