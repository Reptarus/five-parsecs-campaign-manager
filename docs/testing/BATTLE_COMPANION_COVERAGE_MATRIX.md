...
|---|---------|----------|--------|
| F1 | **CONFIRMED BUG.** Objective auto-eval is 8/11 — `check_completion()` handles FIGHT_OFF/ACQUIRE/MOVE_THROUGH/PATROL/DEFEND/SEARCH/PROTECT/DELIVER then `return false`; ACCESS/ELIMINATE/SECURE can never auto-complete → `VictoryProgressPanel` silent. Fix in Phase 6 (add 3 cases + `COVERED_IDS`/config sync). | `MissionObjectiveSystem.gd:92-110`, `BattleObjectiveTracker.gd:19` | CONFIRMED — fix P6 |
...
| F1 | MEDIUM — 3/11 objectives have no auto-eval | ACCESS/ELIMINATE/SECURE fall through `check_completion()` → VictoryProgressPanel silent. JSON defines all 3; only match arms missing. | Characterized (`test_battle_objective_completion` F1 tests); **fix + invert tests in Phase 6** |
