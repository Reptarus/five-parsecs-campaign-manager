...
This guide covers common issues that occur with GUT (Godot Unit Testing) in Godot 4.6+ and how to fix them.
...
**Cause:**
Godot 4.6 removed the ability to create GDScript instances using `GDScript.new()`.
...
3. Create an empty script file that will be loaded instead of creating new GDScript instances:
   ...
   # to replace GDScript.new() functionality in Godot 4.6
   ```
...
1. Update the create_user_preferences function in compatibility.gd:
   ...
   func create_user_preferences(editor_settings):
       # In Godot 4.6, we can't use .new() directly
   ...
   ```
2. Create a simplified gut_user_preferences.gd script that works with Godot 4.6
