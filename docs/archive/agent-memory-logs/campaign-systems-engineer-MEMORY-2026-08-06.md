...
5. **Cross-Mode Character Transfer Framework (SHIPPED — all 4 modes)**: canonical-hub design in `src/core/character/CharacterTransferService.gd` (RefCounted) — the canonical interchange form is the full 5PFH Character dict. Transfer mechanism is direct file-drop at `user://transfers/<id>.json` (v2 envelope), NOT a persistent barracks. Campaign-domain touchpoints: (a) `GameState.load_campaign()` emits `signal pending_character_transfers(count)` on a 5PFH load; (b) mode-generic pickup lives in `src/ui/screens/campaign/CampaignScreenBase.gd` (`_check_pending_transfers`/`_apply_pending_transfers`/`_add_character_to_mode`/`_on_transfers_applied` virtual hook/`_campaign_mode`); each dashboard (CampaignDashboard/BugHuntDashboard/PlanetfallDashboard/TacticsDashboard) calls `_check_pending_transfers.call_deferred()` in `_setup_screen` and overrides `_on_transfers_applied()` to rebuild; the `_add_character_to_mode()` "tactics" case dispatches to `add_veteran_character()` (Jun 4, was a push_warning Phase-2 placeholder); (c) the NEW 5PFH mutation chokepoint `FiveParsecsCampaignCore.add_crew_member(member_dict)` appends to `crew_data["members"]`, forces `is_captain=false`, rebuilds `_crew_id_index` — use this for ANY crew added after creation, never append directly; (d) static `CharacterTransferService.apply_transfer_rewards(campaign, transfer_data)` applies rewards to the receiving campaign and deletes the file (prevents double-import); rewards attach only when `target_mode == "five_parsecs"`. 24/24 gdUnit4 transfer tests. Tactics SHIPPED Jun 4 — a transferred char becomes a NAMED VETERAN in `TacticsCampaignCore.veteran_characters[]` (never a squad unit; out of points validation).
6. **B2 narrative bridge in `CampaignTurnController._on_battle_completed` (May 29 2026)**: auto-resolved battles route through `NarrativeScreen` BEFORE `POST_MISSION`. Gate: `results.get("auto_resolved", false) and _narrative_events_enabled()`. Played-out tactical battles skip the bridge. The producer dict in `_battle_result_to_narrative_dict` MUST use consumer-side keys: `briefing_text` (NOT `briefing`), `held_field` (NOT `held_the_field`). When defaulting a missing key, default to SAFEST value (false), not the convenient one. The helper is `static` so unit tests can call it without instantiating the Control (which would trip @onready % asserts). Test file: `tests/unit/test_b2_narrative_bridge.gd` — 8 bug-pin tests, KEEP GREEN.
7. **Creation→world-phase data-flow (Jun 24 alpha fixit sprint — implemented + compile-clean, ON-DEVICE VERIFY PENDING, uncommitted)**: (a) `CampaignFinalizationService._transform_crew_data_for_turn_system()` now `to_dictionary()`-normalizes Character **Resource** members. A FRESH campaign otherwise held Resources in `crew_data["members"]`, and any consumer using 2-arg `Dictionary.get(key, default)` (CrewTaskComponent `_populate_crew_list`/`_get_eligible_crew`) **silently aborts** on a Resource (Object.get takes 1 arg) → empty crew list → World Phase Step 2 soft-lock. NEW-CAMPAIGN-ONLY (loaded saves are dicts). Crew members are canonically dicts; the turn system already handles dicts (loaded saves prove it). (b) Captain **Luck=0** in review: `update_captain_state()` hand-copies stats but OMITTED `luck` (~L586). RULE: forward the full `to_dictionary()`/`get_panel_data()` at boundaries, never hand-list keys. (c) **Starting credits** (Core Rules p.28 = 1/crew; 6-crew=6cr + roll bonuses) were dropped by the `equipment_generated` signal adapter in `CampaignCreationUI` (no `credits` key → coordinator kept its 0 default). (d) **Victory now REQUIRED**: blocking check in `FinalPanel._validate_campaign_data()` — NOT in `ExpandedConfigPanel._validate_campaign_config()` (it intentionally defers victory to avoid breaking campaign-name propagation). Plan: `jiggly-growing-sunset.md`.
