...
| KeywordDB consumer surfaces | 17 files | PARTIAL COVERAGE | Used by CharacterDetailsScreen, CompendiumScreen, UnifiedBattleLog, BasePhasePanel, EquipmentFormatter, StoryTextFormatter, more. (BattleJournal was also a consumer until it was deleted 2026-09-04 as an orphan — recount before trusting the "17 files".) Coverage gap on TacticalBattleUI enemy cards + PreBattleUI enemy roster (to be verified by Item 2 sweep). |
