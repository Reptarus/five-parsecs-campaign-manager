...
func generate_progression(source: Region, start: Vector2):
	...
	for i in source.progression_rooms.size():
		...
		grid.add_tile(source.progression_rooms[i], Vector2i(x, y))
	...
	for i in source.optional_rooms.size():
		if randf() < float(source.optional_rooms[i].weight) / source.optional_rooms.size():
			...
			grid.add_tile(source.optional_rooms[i], Vector2i(x, y))
	...
	for x in range(-size, size):
		for y in size:
			var pos_x = x + start.x
			var pos_y = y + start.y
			grid.add_tile(source.environment_tiles[randi() % source.environment_tiles.size()], Vector2i(pos_x, pos_y))
