...
func _load_shortcut_from_pref(user_pref):
	...
	if typeof(user_pref.value) == TYPE_DICTIONARY:
		if user_pref.value.has("shortcut") and user_pref.value.shortcut is Array and user_pref.value.shortcut.size() > 0:
			var events = []
			for event_data in user_pref.value.shortcut:
				if event_data is InputEvent:
