...
const PERFORMANCE_THRESHOLD := 16.67 # 60 FPS target
const STABILIZE_TIME := 0.1 # Stabilization time

var _performance_monitor: Node
var _test_battlefield: Node2D
var _enemy_manager: Node
var _created_enemies: Array[Node] = []
...
func test_large_group_creation() -> void:
    ...
    print("Large group creation time: %d ms" % creation_time)
    
    assert_that(group.size()).is_equal(LARGE_GROUP_SIZE)
    assert_that(creation_time).is_less(1000) # Should create 100 enemies in under 1 second
...
func _process_group_ai(group: Array[Node2D]) -> void:
    for enemy in group:
        if enemy.has_method("get_state"):
            enemy.get_state() # Simulate AI processing
...
func _simulate_enemy_combat(enemies: Array[Node2D]) -> void:
    for enemy: Node in enemies:
        if is_instance_valid(enemy):
            ...
            var current_health = enemy.get_meta("health", 10)
...
func verify_performance_metrics(metrics: Dictionary, thresholds: Dictionary) -> void:
    ...
    assert_that(metrics.get("average_frame_time", 0.0)).with_failure_message("Average frame time should be below threshold").is_less(thresholds.get("average_frame_time", 50.0))
    
    assert_that(metrics.get("maximum_frame_time", 0.0)).with_failure_message("Maximum frame time should be below threshold").is_less(thresholds.get("maximum_frame_time", 100.0))
    
    assert_that(metrics.get("memory_delta_kb", 0.0)).with_failure_message("Memory delta should be below threshold").is_less(thresholds.get("memory_delta_kb", 512.0))
