...
var _created_enemies: @warning_ignore("unsafe_call_argument")
	Array[Node2D] = []
...
func before_test() -> void:
	...
	_enemy_manager = @warning_ignore("return_value_discarded")
	auto_free(Node.new())
	...
	_performance_monitor = @warning_ignore("return_value_discarded")
	auto_free(Node.new())
	...
	_test_battlefield = @warning_ignore("return_value_discarded")
...
func _create_enemy_group(size: int) -> Array[Node2D]:
	var enemies: @warning_ignore("unsafe_call_argument")
	Array[Node2D] = []
	for i: int in range(size):
		var enemy = @warning_ignore("return_value_discarded")
	auto_free(Node2D.new())
...
func _simulate_enemy_ai_decisions(enemies: Array[Node2D]) -> void:
	for enemy: Node in enemies:
		if is_instance_valid(enemy):
			...
			enemy.set_meta("ai_state", states[@warning_ignore("integer_division")
	randi() % states.size()])
