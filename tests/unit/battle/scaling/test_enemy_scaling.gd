...
# Mock enemy scaling system for testing
class MockEnemyScalingSystem extends Resource:
    ...
    func calculate_enemy_scaling(difficulty: int, level: int, mission_type: int) -> Dictionary:
        ...
        # Special case: hardcore difficulty in red zone
...
# Helper method to verify scaling results
func _verify_scaling_result(result: Dictionary, expected: Dictionary, message: String) -> void:
    for key in expected:
        assert_that(result.has(key)).with_message(
            "%s - Missing key: %s" % [message, key]
        ).is_true()
        
        var got: float = result.get(key, 0.0)
        var want: float = expected.get(key, 0.0)
        assert_that(got).with_message(
            "%s - %s value mismatch" % [message, key]
        ).is_equal_approx(want, 0.01)
...
func test_base_values() -> void:
    ...
    assert_that(base_health).is_equal(100.0)
    assert_that(base_damage).is_equal(10.0)
    assert_that(base_armor).is_equal(5.0)
    assert_that(base_speed).is_equal(4.0)
...
func test_combined_scaling() -> void:
    ...
    assert_that(scaling["health"]).is_equal_approx(expected_health, 0.01)
    assert_that(scaling["damage"]).is_equal_approx(expected_damage, 0.01)
    assert_that(scaling["count_modifier"]).is_equal_approx(expected_count, 0.01)
...
func test_extreme_scaling_combination() -> void:
    ...
    assert_that(result["health"]).is_less(1000.0)
    assert_that(result["damage"]).is_less(100.0)
