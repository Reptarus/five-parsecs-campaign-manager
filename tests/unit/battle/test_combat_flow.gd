...
func before_each() -> void:
	...
	_game_state = TypeSafeMixin._safe_cast_node(game_state_instance)
	if not _game_state:
	...
	_state_machine = TypeSafeMixin._safe_cast_node(state_machine_instance)
	if not _state_machine:
		push_error("Failed to create battle state machine")
		return
	TypeSafeMixin._safe_method_call_bool(_state_machine, "initialize", [_game_state])
...
func test_battle_phase_transitions() -> void:
	# Test initial state
	var current_phase: int = TypeSafeMixin._safe_method_call_int(_state_machine, "get_current_phase", [])
	assert_eq(current_phase, GameEnums.BattlePhase.NONE, "Should start in NONE phase")
	
	# Start battle
	TypeSafeMixin._safe_method_call_bool(_state_machine, "start_battle", [])
	current_phase = TypeSafeMixin._safe_method_call_int(_state_machine, "get_current_phase", [])
	assert_eq(current_phase, GameEnums.BattlePhase.SETUP, "Should transition to SETUP phase")
	
	# Move through phases
	TypeSafeMixin._safe_method_call_bool(_state_machine, "advance_phase", [])
	current_phase = TypeSafeMixin._safe_method_call_int(_state_machine, "get_current_phase", [])
...
func test_combat_actions() -> void:
	...
	TypeSafeMixin._safe_method_call_bool(_state_machine, "start_battle", [])
	TypeSafeMixin._safe_method_call_bool(_state_machine, "add_combatant", [attacker])
	TypeSafeMixin._safe_method_call_bool(_state_machine, "add_combatant", [defender])
	
	# Move to activation phase
	while TypeSafeMixin._safe_method_call_int(_state_machine, "get_current_phase", []) != GameEnums.BattlePhase.ACTIVATION:
		TypeSafeMixin._safe_method_call_bool(_state_machine, "advance_phase", [])
	...
	var action := {
		...
		"actor": attacker,
		"target": defender,
		"weapon": TypeSafeMixin._safe_method_call_node(attacker, "get_primary_weapon", [])
	}
	
	var attack_result: Dictionary = TypeSafeMixin._safe_method_call_dict(_state_machine, "execute_action", [action])
...
func test_reaction_system() -> void:
	...
	TypeSafeMixin._safe_method_call_bool(_state_machine, "start_battle", [])
	TypeSafeMixin._safe_method_call_bool(_state_machine, "add_combatant", [active_unit])
	TypeSafeMixin._safe_method_call_bool(_state_machine, "add_combatant", [overwatch_unit])
	
	# Setup overwatch
	var overwatch_action := {
		"type": GameEnums.UnitAction.OVERWATCH,
		"actor": overwatch_unit,
		"weapon": TypeSafeMixin._safe_method_call_node(overwatch_unit, "get_primary_weapon", [])
	}
	TypeSafeMixin._safe_method_call_dict(_state_machine, "execute_action", [overwatch_action])
	...
	var move_result: Dictionary = TypeSafeMixin._safe_method_call_dict(_state_machine, "execute_action", [move_action])
...
func test_combat_status_effects() -> void:
	...
	TypeSafeMixin._safe_method_call_bool(_state_machine, "start_battle", [])
	TypeSafeMixin._safe_method_call_bool(_state_machine, "add_combatant", [unit])
	
	# Apply suppression
	TypeSafeMixin._safe_method_call_bool(_state_machine, "apply_status_effect", [unit, GameEnums.CombatStatus.SUPPRESSED])
	var has_status: bool = TypeSafeMixin._safe_method_call_bool(unit, "has_status", [GameEnums.CombatStatus.SUPPRESSED])
	assert_true(has_status, "Unit should be suppressed")
	
	# Test status effect removal
	TypeSafeMixin._safe_method_call_bool(_state_machine, "remove_status_effect", [unit, GameEnums.CombatStatus.SUPPRESSED])
	has_status = TypeSafeMixin._safe_method_call_bool(unit, "has_status", [GameEnums.CombatStatus.SUPPRESSED])
...
func test_combat_tactics() -> void:
	...
	for tactic in tactics:
		TypeSafeMixin._safe_method_call_bool(_state_machine, "set_unit_tactic", [unit, tactic])
		var current_tactic: int = TypeSafeMixin._safe_method_call_int(unit, "get_current_tactic", [])
		assert_eq(current_tactic, tactic, "Unit should have correct tactic")
		
		# Verify combat modifiers
		var modifiers: Dictionary = TypeSafeMixin._safe_method_call_dict(unit, "get_combat_modifiers", [])
		match tactic:
			GameEnums.CombatTactic.AGGRESSIVE:
...
func test_invalid_action_handling() -> void:
	var result: Dictionary = TypeSafeMixin._safe_method_call_dict(_state_machine, "execute_action", [ {}])
	assert_true(result.has("error"), "Should handle empty action")
	
	var invalid_action := {"type": - 1, "actor": null}
	result = TypeSafeMixin._safe_method_call_dict(_state_machine, "execute_action", [invalid_action])
...
func test_action_processing_performance() -> void:
	...
	for i in range(1000):
		...
		TypeSafeMixin._safe_method_call_dict(_state_machine, "execute_action", [action])
...
func _create_test_character(char_name: String) -> Node:
	...
	var character: Node = TypeSafeMixin._safe_cast_node(character_instance)
	if not character:
		push_error("Failed to create test character")
		return null
	
	TypeSafeMixin._safe_method_call_bool(character, "set_character_name", [char_name])
	TypeSafeMixin._safe_method_call_bool(character, "set_max_health", [100])
	TypeSafeMixin._safe_method_call_bool(character, "set_current_health", [100])
