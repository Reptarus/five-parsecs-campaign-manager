...
func before_each() -> void:
	...
	_event_manager = TypeSafeMixin._safe_cast_node(event_manager_instance)
...
func test_battle_event_definitions() -> void:
	var events: Dictionary = TypeSafeMixin._safe_method_call_dict(_event_manager, "get_battle_events", [])
...
func test_critical_hit_event() -> void:
	var event: Dictionary = TypeSafeMixin._safe_method_call_dict(_event_manager, "get_event", ["CRITICAL_HIT"])
...
func test_weapon_jam_event() -> void:
	var event: Dictionary = TypeSafeMixin._safe_method_call_dict(_event_manager, "get_event", ["WEAPON_JAM"])
...
func test_take_cover_event() -> void:
	var event: Dictionary = TypeSafeMixin._safe_method_call_dict(_event_manager, "get_event", ["TAKE_COVER"])
...
func test_check_event_requirements() -> void:
	...
	var critical_result: bool = TypeSafeMixin._safe_method_call_bool(_event_manager, "check_event_requirements", ["CRITICAL_HIT", critical_context])
	assert_true(critical_result, "Should allow critical hit with roll of 6")
	
	critical_context.attack_roll = 5
	critical_result = TypeSafeMixin._safe_method_call_bool(_event_manager, "check_event_requirements", ["CRITICAL_HIT", critical_context])
	...
	var jam_result: bool = TypeSafeMixin._safe_method_call_bool(_event_manager, "check_event_requirements", ["WEAPON_JAM", jam_context])
	assert_true(jam_result, "Should allow weapon jam with ranged weapon and roll of 1")
	
	jam_context.has_ranged_weapon = false
	jam_result = TypeSafeMixin._safe_method_call_bool(_event_manager, "check_event_requirements", ["WEAPON_JAM", jam_context])
	...
	var cover_result: bool = TypeSafeMixin._safe_method_call_bool(_event_manager, "check_event_requirements", ["TAKE_COVER", cover_context])
	assert_true(cover_result, "Should allow taking cover when near cover")
	
	cover_context.near_cover = false
	cover_result = TypeSafeMixin._safe_method_call_bool(_event_manager, "check_event_requirements", ["TAKE_COVER", cover_context])
...
func test_compare_value() -> void:
	...
	compare_result = TypeSafeMixin._safe_method_call_bool(_event_manager, "compare_value", [6, ">=", 6])
	assert_true(compare_result, "6 should be >= 6")
	
	compare_result = TypeSafeMixin._safe_method_call_bool(_event_manager, "compare_value", [7, ">=", 6])
	assert_true(compare_result, "7 should be >= 6")
	
	compare_result = TypeSafeMixin._safe_method_call_bool(_event_manager, "compare_value", [5, ">=", 6])
	assert_false(compare_result, "5 should not be >= 6")
	
	compare_result = TypeSafeMixin._safe_method_call_bool(_event_manager, "compare_value", [1, "<=", 1])
	assert_true(compare_result, "1 should be <= 1")
	
	compare_result = TypeSafeMixin._safe_method_call_bool(_event_manager, "compare_value", [0, "<=", 1])
	assert_true(compare_result, "0 should be <= 1")
	
	compare_result = TypeSafeMixin._safe_method_call_bool(_event_manager, "compare_value", [2, "<=", 1])
	assert_false(compare_result, "2 should not be <= 1")

# Error Handling Tests
func test_invalid_event_handling() -> void:
	var result: Dictionary = TypeSafeMixin._safe_method_call_dict(_event_manager, "get_event", ["INVALID_EVENT"])
	assert_null(result, "Should handle invalid event gracefully")
	
	var check_result: bool = TypeSafeMixin._safe_method_call_bool(_event_manager, "check_event_requirements", ["INVALID_EVENT", {}])
...
func test_event_processing_performance() -> void:
	...
	for i in range(1000):
		TypeSafeMixin._safe_method_call_bool(_event_manager, "check_event_requirements", ["CRITICAL_HIT", context])
		TypeSafeMixin._safe_method_call_bool(_event_manager, "check_event_requirements", ["WEAPON_JAM", context])
		TypeSafeMixin._safe_method_call_bool(_event_manager, "check_event_requirements", ["TAKE_COVER", context])
