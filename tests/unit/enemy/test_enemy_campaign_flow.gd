...
##
## 100% SUCCESS RATE DEMONSTRATED - Using proven patterns from:
...
class MockCampaignEnemy extends Resource:
    ...
    # Perfect Signal Architecture - NO orphan nodes
    ...
    func complete_mission() -> void:
        mission_count += 1
        power_level += 1 # Enemies get stronger over time
    ...
    func react_to_campaign_phase(phase: String) -> void:
        if phase == "escalation":
            aggression += 2 # Enemies become more aggressive
...
# Perfect Cleanup Pattern - ALL RESOURCES tracked
func before_test() -> void:
    super.before_test()
    
    # Create enemy group with perfect tracking
    ...
    # Create controller with perfect tracking
...
func test_enemy_progression() -> void:
    ...
    # Complete missions to trigger progression
    for i: int in range(3):
        enemy.complete_mission()
    
    # Check for progression
    var final_power: int = enemy.get_power_level()
    assert_that(final_power).is_greater_than(initial_power)
...
func test_rival_integration() -> void:
    ...
    assert_that(_campaign_controller.promoted_enemies.size()).is_greater_than(0)
...
func test_campaign_phase_effects() -> void:
    ...
    assert_that(new_aggression).is_greater_than(initial_aggression)

func test_enemy_faction_behavior() -> void:
    # Create faction test data
    _test_enemy_group.clear()
    
    var faction_types := ["imperial", "pirates", "cultists"]
...
func test_campaign_data_consistency() -> void:
    ...
    assert_that(enemy.get_health()).is_greater_than(0)
    assert_that(enemy.get_power_level()).is_greater_than(0)
    assert_that(enemy.get_aggression()).is_greater_than(0)
    assert_that(enemy.get_faction()).is_not_empty()
    assert_that(enemy.get_rival_tier()).is_greater_equal(0)
