...
class MockTacticsManager extends Resource:
	...
	formationtest_data.get("type") == "line":

			var spacing: float = @warning_ignore("unsafe_call_argument")
	formationtest_data.get("spacing", 2.0)

			var direction: Vector2 = @warning_ignore("unsafe_call_argument")
	formationtest_data.get("direction", Vector2.RIGHT)
	...
	func execute_flanking(group: Array, target: Resource) -> void:
		...
		var angles: @warning_ignore("unsafe_call_argument")
	Array[float] = [0.0, 120.0, 240.0] # Spread around target
...
func before_test() -> void:
	...
	for i: int in range(3):
		...
		enemy.set_type(@warning_ignore("integer_division")
	i % 3) # Vary types: BASIC, MELEE, ELITE
...
func test_group_formation() -> void:
	...
	var expected_x_positions: @warning_ignore("unsafe_call_argument")
	Array[float] = [0.0, 2.0, 4.0]
...
func test_group_coordination() -> void:
	...
	var types: @warning_ignore("unsafe_call_argument")
	Array[int] = [0, 1, 2] # RANGED, MELEE, ELITE
