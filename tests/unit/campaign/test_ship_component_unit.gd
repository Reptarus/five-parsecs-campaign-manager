...
class MockShipComponent extends Resource:
    ...
    func set_durability(value: int) -> void:
        durability = clamp(value, 0, 100)
    
    func set_efficiency(value: float) -> void:
        efficiency = clamp(value, 0.0, 2.0)
...
func test_initialization() -> void:
    ...
    assert_that(name).is_equal("Test Component")
    assert_that(description).is_not_empty()
    assert_that(cost).is_equal(100)
    assert_that(power_draw).is_equal(25)
...
func test_efficiency_modification() -> void:
    ...
    # Test efficiency boost (clamped)
    component.set_efficiency(1.5)
    current_efficiency = component.get_efficiency()
    assert_that(current_efficiency).is_equal(1.5)
    
    # Test negative efficiency (clamped)
...
func test_edge_cases() -> void:
    ...
    # Test repair when at full durability
    component.set_durability(100)
    success = component.repair_component(10)
    assert_that(success).is_false()
    
    # Test upgrade limit
    for i: int in range(15): # Try to upgrade beyond max
        component.upgrade_level()
    
    var final_level: int = component.get_level()
    assert_that(final_level).is_less_equal(10) # Should not exceed maximum
    
func test_component_properties() -> void:
    # Test all core properties are accessible
    assert_that(component.get_component_name()).is_not_empty()
    assert_that(component.get_description()).is_not_empty()
    assert_that(component.get_cost()).is_greater(0)
    assert_that(component.get_power_draw()).is_greater_equal(0)
    assert_that(component.get_level()).is_greater(0)
    assert_that(component.get_durability()).is_between(0, 100)
    assert_that(component.get_efficiency()).is_greater_equal(0.0)
    assert_that(component.get_component_type()).is_greater_equal(0)
