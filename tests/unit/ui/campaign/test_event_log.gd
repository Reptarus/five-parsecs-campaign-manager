...
func before_each() -> void:
	...
	if not _event_log:
		push_error("Failed to create event log")
		return
	TypeSafeMixin._safe_method_call_bool(_event_log, "initialize", [])
	add_child_autofree(_event_log)
	track_test_node(_event_log)
...
func test_event_creation() -> void:
	...
	var success: bool = TypeSafeMixin._safe_method_call_bool(_event_log, "add_event", [event_data])
	assert_true(success, "Should add event successfully")
	verify_signal_emitted(_event_log, "event_added")
	
	var events: Array = TypeSafeMixin._safe_method_call_array(_event_log, "get_events", [])
...
func test_event_filtering() -> void:
	...
	var events := [
		...
		{
			"type": GameEnums.EventCategory.COMBAT,
			"description": "Combat event 2",
			"timestamp": Time.get_unix_time_from_system()
		}
	]
	
	for event in events:
		TypeSafeMixin._safe_method_call_bool(_event_log, "add_event", [event])
	
	# Filter by type
	var combat_events: Array = TypeSafeMixin._safe_method_call_array(_event_log, "get_events_by_type", [GameEnums.EventCategory.COMBAT])
	assert_eq(combat_events.size(), 2, "Should have two combat events")
	
	var equipment_events: Array = TypeSafeMixin._safe_method_call_array(_event_log, "get_events_by_type", [GameEnums.EventCategory.EQUIPMENT])
...
func test_event_sorting() -> void:
	...
	var events := [
		{
			...
			"timestamp": Time.get_unix_time_from_system() - 100
		},
		{
			"type": GameEnums.EventCategory.COMBAT,
	...
	var sorted_events: Array = TypeSafeMixin._safe_method_call_array(_event_log, "get_sorted_events", [])
...
func test_event_clearing() -> void:
	...
	TypeSafeMixin._safe_method_call_bool(_event_log, "add_event", [event_data])
	
	# Clear events
	TypeSafeMixin._safe_method_call_bool(_event_log, "clear_events", [])
...
func test_event_validation() -> void:
	watch_signals(_event_log)
	...
	var success: bool = TypeSafeMixin._safe_method_call_bool(_event_log, "add_event", [invalid_event])
	assert_false(success, "Should not add invalid event")
	verify_signal_not_emitted(_event_log, "event_added")
	
	# Test null event data
	success = TypeSafeMixin._safe_method_call_bool(_event_log, "add_event", [null])
...
func test_event_search() -> void:
	...
	# Search by keyword
	var combat_results: Array = TypeSafeMixin._safe_method_call_array(_event_log, "search_events", ["pirates"])
	assert_eq(combat_results.size(), 1, "Should find one event with 'pirates'")
	
	var weapon_results: Array = TypeSafeMixin._safe_method_call_array(_event_log, "search_events", ["weapon"])
...
func test_event_persistence() -> void:
	...
	# Save events
	var save_success: bool = TypeSafeMixin._safe_method_call_bool(_event_log, "save_events", [])
	...
	TypeSafeMixin._safe_method_call_bool(_event_log, "clear_events", [])
	var load_success: bool = TypeSafeMixin._safe_method_call_bool(_event_log, "load_events", [])
	assert_true(load_success, "Should load events successfully")
	verify_signal_emitted(_event_log, "events_loaded")
	
	var loaded_events: Array = TypeSafeMixin._safe_method_call_array(_event_log, "get_events", [])
...
func test_error_handling() -> void:
	...
	var success: bool = TypeSafeMixin._safe_method_call_bool(_event_log, "add_event", [incomplete_event])
	assert_false(success, "Should not add event with missing fields")
	verify_signal_not_emitted(_event_log, "event_added")
	
	# Test loading from invalid source
	success = TypeSafeMixin._safe_method_call_bool(_event_log, "load_events_from_file", ["invalid_path.json"])
