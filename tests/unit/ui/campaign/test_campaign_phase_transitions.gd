...
func before_each() -> void:
	...
	if not _phase_manager:
		...
		return
	TypeSafeMixin._safe_method_call_bool(_phase_manager, "initialize", [_game_state])
...
func test_initial_phase() -> void:
	var phase: int = TypeSafeMixin._safe_method_call_int(_phase_manager, "get_current_phase", [])
...
func test_basic_phase_transition() -> void:
	...
	var success: bool = TypeSafeMixin._safe_method_call_bool(_phase_manager, "transition_to", [GameEnums.FiveParcsecsCampaignPhase.UPKEEP])
	assert_true(success, "Should transition to UPKEEP phase")
	
	var current_phase: int = TypeSafeMixin._safe_method_call_int(_phase_manager, "get_current_phase", [])
...
func test_invalid_phase_transition() -> void:
	...
	var success: bool = TypeSafeMixin._safe_method_call_bool(_phase_manager, "transition_to", [-1])
...
func test_upkeep_phase() -> void:
	...
	TypeSafeMixin._safe_method_call_bool(_phase_manager, "transition_to", [GameEnums.FiveParcsecsCampaignPhase.UPKEEP])
	
	# Test upkeep actions
	var upkeep_result: Dictionary = TypeSafeMixin._safe_method_call_dict(_phase_manager, "process_upkeep", [])
...
func test_story_phase() -> void:
	...
	TypeSafeMixin._safe_method_call_bool(_phase_manager, "transition_to", [GameEnums.FiveParcsecsCampaignPhase.STORY])
	
	# Test story event generation
	var story_event: Dictionary = TypeSafeMixin._safe_method_call_dict(_phase_manager, "generate_story_event", [])
...
func test_battle_setup_phase() -> void:
	...
	TypeSafeMixin._safe_method_call_bool(_phase_manager, "transition_to", [GameEnums.FiveParcsecsCampaignPhase.BATTLE_SETUP])
	
	# Test battle initialization
	var battle_state: Dictionary = TypeSafeMixin._safe_method_call_dict(_phase_manager, "initialize_battle", [])
...
func test_battle_resolution_phase() -> void:
	...
	TypeSafeMixin._safe_method_call_bool(_phase_manager, "transition_to", [GameEnums.FiveParcsecsCampaignPhase.BATTLE_RESOLUTION])
	
	# Test battle resolution
	var resolution: Dictionary = TypeSafeMixin._safe_method_call_dict(_phase_manager, "resolve_battle", [])
...
func test_full_phase_sequence() -> void:
	...
	for phase in phases:
		var success: bool = TypeSafeMixin._safe_method_call_bool(_phase_manager, "transition_to", [phase])
...
func test_phase_prerequisites() -> void:
	...
	var success: bool = TypeSafeMixin._safe_method_call_bool(_phase_manager, "transition_to", [GameEnums.FiveParcsecsCampaignPhase.BATTLE_SETUP])
	...
	success = TypeSafeMixin._safe_method_call_bool(_phase_manager, "transition_to", [GameEnums.FiveParcsecsCampaignPhase.BATTLE_RESOLUTION])
...
func test_phase_state_persistence() -> void:
	...
	TypeSafeMixin._safe_method_call_bool(_phase_manager, "transition_to", [GameEnums.FiveParcsecsCampaignPhase.UPKEEP])
	TypeSafeMixin._safe_method_call_bool(_phase_manager, "set_phase_data", [ {"resources": 100}])
	...
	var phase_data: Dictionary = TypeSafeMixin._safe_method_call_dict(_phase_manager, "get_phase_data", [])
...
func test_error_handling() -> void:
	...
	var success: bool = TypeSafeMixin._safe_method_call_bool(_phase_manager, "set_phase_data", [null])
	assert_false(success, "Should handle null phase data gracefully")
	
	# Test invalid phase transition
	success = TypeSafeMixin._safe_method_call_bool(_phase_manager, "transition_to", [999])
	assert_false(success, "Should handle invalid phase transition gracefully")
	
	# Test missing prerequisites
	success = TypeSafeMixin._safe_method_call_bool(_phase_manager, "process_battle", [])
