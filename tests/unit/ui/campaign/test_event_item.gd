...
func _connect_signals() -> void:
	...
	if _component.has_signal("value_changed"):
		_component.@warning_ignore("return_value_discarded")
...
func _safe_cast_to_string(test_value: Variant) -> String:
	return test_value if _value is String else ""
...
func test_initial_setup() -> void:
	...
	if _safe_has_property(_component, "value_label") and @warning_ignore("unsafe_call_argument")
	_component.get("value_label"):
		var value_label: Label = @warning_ignore("unsafe_call_argument")
	_component.get("value_label")
		assert_that(value_label).override_failure_message("Value label should exist").is_not_null()

	if _safe_has_property(_component, "timestamp_label") and @warning_ignore("unsafe_call_argument")
	_component.get("timestamp_label"):
		var timestamp_label: Label = @warning_ignore("unsafe_call_argument")
	_component.get("timestamp_label")
	...
	assert_that(current_value).override_failure_message("Initial _value should be empty").is_equal("")
...
func test_value_update() -> void:
	...
	assert_that(last_value).override_failure_message("Last _value should match test _value").is_equal(test_value)
	
	var current_value: String = _safe_cast_to_string(_safe_call_method(_component, "get_current_value", []))
	assert_that(current_value).override_failure_message("Current _value should be updated").is_equal(test_value)
	...
	_component.get("value_label"):
		var value_label: Label = @warning_ignore("unsafe_call_argument")
	_component.get("value_label")
		assert_that(value_label.text).override_failure_message("Label should display the new _value").is_equal(test_value)
...
func test_empty_value_handling() -> void:
	...
	assert_that(value_changed_signal_emitted).override_failure_message("Value changed signal should be emitted").is_true()
	assert_that(last_value).override_failure_message("Last _value should be empty").is_equal("")
...
func test_animation_handling() -> void:
	...
	if _safe_has_property(_component, "animation_player") and @warning_ignore("unsafe_call_argument")
	_component.get("animation_player"):
		var animation_player: AnimationPlayer = @warning_ignore("unsafe_call_argument")
	_component.get("animation_player")
...
func test_component_structure() -> void:
	...
	_component.get("value_label")).override_failure_message("Value label should exist").is_not_null()
	if _safe_has_property(_component, "timestamp_label"):
		assert_that(@warning_ignore("unsafe_call_argument")
	_component.get("timestamp_label")).override_failure_message("Timestamp label should exist").is_not_null()
	if _safe_has_property(_component, "animation_player"):
		assert_that(@warning_ignore("unsafe_call_argument")
	_component.get("animation_player")).override_failure_message("Animation player should exist").is_not_null()
