...
# Define SwipeDirection enum since it's not in the GestureManager class
enum SwipeDirection {
    LEFT,
    RIGHT,
    UP,
...
func before_each() -> void:
    ...
    _gesture_manager = GestureManager.new()
    if not _gesture_manager:
        push_error("Failed to create gesture manager")
        return
    add_child(_gesture_manager)
    track_test_node(_gesture_manager)
    await _gesture_manager.ready
...
func after_each() -> void:
    if is_instance_valid(_gesture_manager):
        _gesture_manager.queue_free()
    _gesture_manager = null
    
    if is_instance_valid(_test_node):
...
func test_tap_recognition() -> void:
    ...
    verify_signal_emitted(_gesture_manager, "tap")
...
func test_swipe_recognition() -> void:
    ...
    verify_signal_emitted(_gesture_manager, "swipe")
    ...
    assert_eq(last_signal_data[3], SwipeDirection.RIGHT, "Swipe direction should be RIGHT")
...
func test_pinch_recognition() -> void:
    ...
    verify_signal_emitted(_gesture_manager, "pinch")
...
func test_gesture_settings() -> void:
    ...
    if not ("tap_timeout" in _gesture_manager and
            "swipe_threshold" in _gesture_manager and
            "pinch_threshold" in _gesture_manager):
        push_warning("Skipping settings verification: required properties not found")
...
func test_multi_touch_handling() -> void:
    ...
    if not "active_touches" in _gesture_manager:
        push_warning("Skipping active_touches check: property not found")
        pending("Test skipped - active_touches property not found")
        return
...
func test_cleanup() -> void:
    ...
    if not (_gesture_manager.has_method("is_target_registered") and
            "active_touches" in _gesture_manager):
        push_warning("Skipping cleanup verification: required methods or properties not found")
