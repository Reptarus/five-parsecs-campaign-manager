...
func test_tab_switching() -> void:
    ...
    assert_that(mock_dialog.get_current_tab()).is_equal(1) # Should stay at last valid tab
...
func test_validate_settings() -> void:
    ...
    #
    mock_dialog.update_setting("volume", 1.5) # Invalid value
...
func test_invalid_operations() -> void:
    ...
    #
    mock_dialog.switch_tab(-1)
    assert_that(mock_dialog.get_current_tab()).is_equal(0) # Should stay at 0
