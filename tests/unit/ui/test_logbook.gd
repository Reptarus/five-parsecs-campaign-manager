...
class MockLogbook extends Resource:
	...
	func get_total_pages() -> int:
		return max(1, (entries.size() + entries_per_page - 1) / entries_per_page)
...
func before_test() -> void:
	...
	auto_free(mock_logbook) # Perfect cleanup
...
func test_add_entry() -> void:
	...
	assert_that(mock_logbook.get_entry_count()).is_equal(1)
...
func test_remove_entry() -> void:
	...
	assert_that(mock_logbook.get_entry_count()).is_equal(0)
...
func test_get_entry() -> void:
	...
	assert_that(retrieved_entry).is_not_empty()
...
func test_select_entry() -> void:
	...
	assert_that(result).is_true()
	assert_that(mock_logbook.selected_entry).is_not_empty()

func test_pagination() -> void:
	# Add multiple entries to test pagination
...
func test_entry_types_and_categories() -> void:
	...
	assert_that(mock_logbook.entry_types).contains("encounter")
	assert_that(mock_logbook.log_categories).contains("combat")
	assert_that(mock_logbook.log_categories).contains("diplomacy")
...
func test_empty_search() -> void:
	...
	assert_that(results).is_equal(mock_logbook.entries)

func test_log_entry_addition() -> void:
	# Test entry addition functionality
