...
func test_panel_initialization() -> void:
    ...
    assert_that(mock_panel.state_categories.size()).is_equal(5)
...
func test_state_properties() -> void:
    ...
    assert_that(mock_panel.is_verifying).is_false()
    assert_that(mock_panel.correction_requested).is_false()

func test_state_categories() -> void:
    var categories = mock_panel.get_state_categories()
    assert_that(categories.size()).is_equal(5)
    assert_that(categories).contains("Combat")
    assert_that(categories).contains("Position")
    assert_that(categories).contains("Resources")
    assert_that(categories).contains("Effects")
    assert_that(categories).contains("Modifiers")
...
func test_auto_verify_functionality() -> void:
    ...
    assert_signal(mock_panel).is_emitted("auto_verify_changed")
...
func test_auto_verify_checkbox() -> void:
    ...
    assert_signal(auto_verify_check).is_emitted("toggled")
...
func test_manual_correction_request() -> void:
    ...
    assert_signal(mock_panel).is_emitted("manual_correction_requested")
...
func test_export_verification_results() -> void:
    ...
    assert_that(results.has("exported_at")).is_true()
    assert_that(results.has("timestamp")).is_true()
...
func test_validator_assignment() -> void:
    #
    var new_validator: Resource = Resource.new() #
