...
class MockUIManager extends Resource:
    ...
    var available_screens: Array[String] = [
        "main_menu", "campaign", "combat", "settings", "save_load", "character_sheet", "inventory", "trading", "mission_select"
    ]
    ...
    func save_ui_state() -> void:
        # Mock implementation - no actual state saving needed in tests
        saved_state = {
            "current_screen": current_screen,
            "ui_scale": ui_scale,
            "theme": theme_name
...
func before_test() -> void:
    ...
    auto_free(mock_ui_manager) # Perfect cleanup
...
func test_clear_history() -> void:
    ...
    assert_that(mock_ui_manager.can_go_back()).is_true() # Still can go back to previous
