...
class MockCampaignSetupScreen extends Control:
    ...
    var campaign_config = {
        "name": ": ","difficulty_level": 1, # Normal difficulty
    ...
    func is_permadeath_forced() -> bool:
        return campaign_config.difficulty_level == 3

    func is_permadeath_disabled() -> bool:
        return campaign_config.difficulty_level == 0
    
    func _update_start_button_state() -> void:
        is_start_button_enabled = campaign_config["name"] != ""
...
func test_campaign_config_defaults() -> void:
    ...
    assert_that(_instance.campaign_config["difficulty_level"]).is_equal(1) #
