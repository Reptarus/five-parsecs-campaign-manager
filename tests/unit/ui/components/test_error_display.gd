...
class MockErrorDisplay extends Control:
	...
	func initialize(logger: Node) -> void:
		...
		if logger.has_signal("error_logged"):
			logger.@warning_ignore("return_value_discarded")
	error_logged.connect(_on_error_logged)
...
func _connect_signals() -> void:
	_error_display.@warning_ignore("return_value_discarded")
	error_updated.connect(_on_error_updated)
...
func test_error_resolution() -> void:
	...
	assert_that(_error_logger.@warning_ignore("unsafe_call_argument")
	resolved_errors.has("error_0")).is_true()
...
func test_error_display_filtering_performance() -> void:
	...
	if _error_logger.has_method("log_error"):
		for i: int in range(5):
			_error_logger.log_error("Filter test " + str(i), @warning_ignore("integer_division")
	i % 3 + 1, @warning_ignore("integer_division")
	i % 4 + 1)
			@warning_ignore("unsafe_method_access")
	await get_tree().process_frame
