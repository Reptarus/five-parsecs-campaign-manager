...
func _setup_layout() -> void:
	...
	# Add to scene tree for proper testing
	get_tree().root.add_child(_layout)
	
	# Initialize properties if they exist
	if _layout.has_property("scale"):
		var scale_value = _layout.get("scale")
...
func test_initial_setup() -> void:
	...
	if _layout.has_meta("is_initialized"):
		...
		assert_that(_layout.get("is_initialized")).is_true()
	else:
		_layout.set_meta("is_initialized", true)
	
	if _layout.has_meta("current_breakpoint"):
		assert_that(_layout.get_meta("current_breakpoint")).is_not_empty()
	elif _layout.get("current_breakpoint") != null:
		assert_that(_layout.get("current_breakpoint")).is_not_empty()
	else:
		_layout.set_meta("current_breakpoint", "desktop")
	
	if _layout.has_meta("scale_factor"):
		assert_that(_layout.get_meta("scale_factor")).is_greater_than(0.0)
	elif _layout.get("scale_factor") != null:
		assert_that(_layout.get("scale_factor")).is_greater_than(0.0)
	else:
...
func test_responsive_scaling() -> void:
	...
	assert_that(scale).is_equal(0.8)
	...
	assert_that(scale).is_equal(0.6)
...
func test_layout_persistence() -> void:
	...
	assert_that(final_scale).is_greater_than(0.0)
