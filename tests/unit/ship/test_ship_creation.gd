...
class MockShipCreation extends Resource:
    ...
    func create_ship(ship_data: Dictionary) -> Resource:
        ...
        ship.hull_points = ship_data.get("hull_points", 0)
        ship.shield_points = ship_data.get("shield_points", 0)
    ...
    func create_component(component_data: Dictionary) -> Resource:
        ...
        var type_value = component_data.get("type", -1)
...
func test_component_limits() -> void:
    ...
    var ship_data: Dictionary = {
        ...
        "hull_points": 200,
        "shield_points": 100,
        "components": [component_data, component_data, component_data]
    }
    ...
    var components: Array = ship.get_components()
    assert_that(components.size()).is_equa