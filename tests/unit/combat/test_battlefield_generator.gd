...
# Test data
var _test_battlefield: BattlefieldData
var _test_mission: TestMission
...
func _create_test_mission(type: int = GameEnums.MissionType.PATROL) -> TestMission:
	if not _instance.has_method("generate_mission"):
...
func _generate_large_battlefield() -> BattlefieldData:
	var config: Dictionary = {"size": STRESS_TEST_SIZE}
	var battlefield_dict := _instance.generate_battlefield(config)
...
func before_each() -> void:
	...
	# Add child node and verify it was added successfully
	var added_node := add_child_autofree(_instance)
	if not added_node or not is_instance_valid(added_node):
		push_error("Failed to add child node")
	track_test_node(_instance)
	track_test_resource(_terrain_rules)
...
# Helper method to create and track battlefield
func _create_battlefield(config: Dictionary) -> BattlefieldData:
	if not _instance.has_method("generate_battlefield"):
		push_error("BattlefieldGenerator missing generate_battlefield method")
		return null
	
	var battlefield_dict: Dictionary = _instance.generate_battlefield(config)
	if battlefield_dict.is_empty():
		return null
		
	return _convert_battlefield_data(battlefield_dict)

# Helper method to convert terrain data
...
# Helper method to safely convert Dictionary to BattlefieldData
...
# Helper method to convert array to Vector2 array
...
func test_cover_generation() -> void:
	...
	# Use floating-point division to avoid integer division warning
...
func test_line_of_sight_paths() -> void:
	...
	if player_zone.size() > 0 and enemy_zone.size() > 0:
		if not _instance.has_method("find_clear_paths"):
			push_error("BattlefieldGenerator missing find_clear_paths method")
			return
			
		var paths_array: Array = _instance.call("find_clear_paths", player_zone[0], enemy_zone[0])
		var paths: Array[Vector2] = _convert_vector2_array(paths_array)
	...
	for feature: TerrainData in terrain:
		...
		match feature_type:
			TerrainTypes.Type.COVER_HIGH:
				var modifiers: Array[int] = _terrain_rules.get_feature_modifiers(GameEnums.TerrainFeatureType.OBSTACLE)
			...
			TerrainTypes.Type.COVER_LOW:
				var modifiers: Array[int] = _terrain_rules.get_feature_modifiers(GameEnums.TerrainFeatureType.COVER)
			...
			TerrainTypes.Type.HAZARD:
				var modifiers: Array[int] = _terrain_rules.get_feature_modifiers(GameEnums.TerrainFeatureType.HAZARD)
...
func test_environment_types() -> void:
	...
	var urban_modifiers: Array[int] = _terrain_rules.get_terrain_modifiers(GameEnums.PlanetEnvironment.URBAN)
	...
	if not forest_battlefield:
		...
		return
	var forest_modifiers: Array[int] = _terrain_rules.get_terrain_modifiers(GameEnums.PlanetEnvironment.FOREST)
	...
	if not hazard_battlefield:
		...
		return
	var hazard_modifiers: Array[int] = _terrain_rules.get_terrain_modifiers(GameEnums.PlanetEnvironment.HAZARDOUS)
...
func test_battlefield_size() -> void:
	...
	if not _instance.has_method("get_battlefield_size"):
		push_error("BattlefieldGenerator missing get_battlefield_size method")
		return
		
	var size_data: Vector2i = _instance.call("get_battlefield_size", mission)
...
func test_battlefield_size_by_mission_type() -> void:
	...
	for type: int in mission_types:
		var mission: TestMission = _create_test_mission(type)
		var size: Vector2i = _instance.call("get_battlefield_size", mission)
		assert_true(size.x > 0 and size.y > 0,
			"Battlefield for mission type %s should have positive dimensions" % GameEnums.MissionType.keys()[type])
		assert_true(size.x <= DEFAULT_MAX_SIZE.x and size.y <= DEFAULT_MAX_SIZE.y,
			"Battlefield for mission type %s should not exceed maximum size" % GameEnums.MissionType.keys()[type])
		assert_true(size.x >= DEFAULT_MIN_SIZE.x and size.y >= DEFAULT_MIN_SIZE.y,
			"Battlefield for mission type %s should meet minimum size requirements" % GameEnums.MissionType.keys()[type])
...
func test_error_invalid_mission_type() -> void:
	...
	if not _instance.has_method("generate_battlefield_for_mission"):
		push_error("BattlefieldGenerator missing generate_battlefield_for_mission method")
		return
		
	var result_data: Dictionary = _instance.call("generate_battlefield_for_mission", mission)
	var result: BattlefieldData = null
	if not result_data.is_empty():
		result = _convert_battlefield_data(result_data)
	assert_null(result, "Invalid mission type should return null")