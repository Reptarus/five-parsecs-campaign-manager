...
class MockTerrainLayout extends Resource:
	...
	func is_valid() -> bool:
		return is_initialized and width > 0 and height > 0 and tiles.size() == width
...
func test_out_of_bounds_handling() -> void:
	...
	assert_that(_layout.set_tile_type(Vector2i(4, 0), TerrainTypes.Type.WALL)).is_false()
	assert_that(_layout.set_tile_type(Vector2i(0, 4), TerrainTypes.Type.WALL)).is_false()
	
	# Test getting tiles from invalid positions
	assert_that(_layout.get_tile_type(Vector2i(-1, 0))).is_equal(-1)
	assert_that(_layout.get_tile_type(Vector2i(5, 5))).is_equal(-1)
...
func test_walkability_queries() -> void:
	...
	# Test walkability
	assert_that(_layout.is_tile_walkable(Vector2i(2, 2))).is_false() # Wall
	assert_that(_layout.is_tile_walkable(Vector2i(1, 1))).is_true() # Cover
	assert_that(_layout.is_tile_walkable(Vector2i(3, 3))).is_true() # Difficult
	assert_that(_layout.is_tile_walkable(Vector2i(0, 0))).is_true() # Empty
...
func test_line_of_sight_queries() -> void:
	...
	assert_that(_layout.blocks_line_of_sight(Vector2i(2, 2))).is_true() # Wall blocks
	assert_that(_layout.blocks_line_of_sight(Vector2i(1, 1))).is_true() # High cover blocks
	assert_that(_layout.blocks_line_of_sight(Vector2i(0, 0))).is_false() # Empty doesn't block
...
func test_area_queries() -> void:
	...
	assert_that(area_tiles).contains(Vector2i(2, 2))
	assert_that(area_tiles).contains(Vector2i(3, 3))
	assert_that(area_tiles).contains(Vector2i(4, 4))
...
func test_layout_validation() -> void:
	...
	assert_that(walkable_tiles).not_contains(Vector2i(2, 2)) # Wall position
	assert_that(walkable_tiles).contains(Vector2i(3, 3)) # Cover position
...
func test_flood_fill_operations() -> void:
	...
	assert_that(connected_tiles).contains(start_pos)
