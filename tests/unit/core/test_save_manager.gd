...
func test_initialization() -> void:
    ...
    var save_directory: String = TypeSafeMixin._safe_method_call_string(save_manager, "get_save_directory", [], "")
    var auto_save_enabled: bool = TypeSafeMixin._safe_method_call_bool(save_manager, "is_auto_save_enabled", [], false)
    var auto_save_interval: int = TypeSafeMixin._safe_method_call_int(save_manager, "get_auto_save_interval", [], 0)
...
func test_save_file_management() -> void:
    ...
    var success: bool = TypeSafeMixin._safe_method_call_bool(save_manager, "create_save", ["test_save", test_data], false)
    assert_true(success, "Should create save file")
    
    # Test loading save file
    var loaded_data: Dictionary = TypeSafeMixin._safe_method_call_dict(save_manager, "load_save", ["test_save"], {})
    ...
    var save_files: Array = TypeSafeMixin._safe_method_call_array(save_manager, "get_save_files", [], [])
    assert_true("test_save" in save_files, "Should list created save file")
    
    # Test deleting save file
    success = TypeSafeMixin._safe_method_call_bool(save_manager, "delete_save", ["test_save"], false)
    assert_true(success, "Should delete save file")
    
    save_files = TypeSafeMixin._safe_method_call_array(save_manager, "get_save_files", [], [])
...
func test_auto_save_functionality() -> void:
    ...
    TypeSafeMixin._safe_method_call_bool(save_manager, "set_auto_save_enabled", [false])
    var auto_save_enabled: bool = TypeSafeMixin._safe_method_call_bool(save_manager, "is_auto_save_enabled", [], true)
    assert_false(auto_save_enabled, "Should disable auto-save")
    
    TypeSafeMixin._safe_method_call_bool(save_manager, "set_auto_save_interval", [30])
    var auto_save_interval: int = TypeSafeMixin._safe_method_call_int(save_manager, "get_auto_save_interval", [], 0)
    assert_eq(auto_save_interval, 30, "Should update auto-save interval")
    
    # Test auto-save trigger
    TypeSafeMixin._safe_method_call_bool(save_manager, "set_auto_save_enabled", [true])
    var test_data := {"auto_save_test": true}
    TypeSafeMixin._safe_method_call_bool(save_manager, "update_auto_save_data", [test_data])
    
    await get_tree().create_timer(auto_save_interval + 1).timeout
    
    var auto_save_files: Array = TypeSafeMixin._safe_method_call_array(save_manager, "get_auto_save_files", [], [])
    assert_gt(auto_save_files.size(), 0, "Should create auto-save file")
    
    var loaded_auto_save: Dictionary = TypeSafeMixin._safe_method_call_dict(save_manager, "load_auto_save", [auto_save_files[0]], {})
...
func test_save_data_validation() -> void:
    ...
    var success: bool = TypeSafeMixin._safe_method_call_bool(save_manager, "create_save", ["invalid_save", invalid_data], false)
    assert_false(success, "Should reject invalid save data")
    
    # Test empty save name
    success = TypeSafeMixin._safe_method_call_bool(save_manager, "create_save", ["", {"valid": "data"}], false)
    assert_false(success, "Should reject empty save name")
    
    # Test invalid characters in save name
    success = TypeSafeMixin._safe_method_call_bool(save_manager, "create_save", ["invalid/save\\name", {"valid": "data"}], false)
    assert_false(success, "Should reject invalid save name characters")
    
    # Test loading non-existent save
    var loaded_data: Dictionary = TypeSafeMixin._safe_method_call_dict(save_manager, "load_save", ["non_existent_save"], {})
...
func test_save_backup_management() -> void:
    ...
    var success: bool = TypeSafeMixin._safe_method_call_bool(save_manager, "create_backup", ["test_save", test_data], false)
    assert_true(success, "Should create backup")
    
    # Test listing backups
    var backups: Array = TypeSafeMixin._safe_method_call_array(save_manager, "get_backups", ["test_save"], [])
    assert_gt(backups.size(), 0, "Should list created backup")
    
    # Test loading backup
    var loaded_backup: Dictionary = TypeSafeMixin._safe_method_call_dict(save_manager, "load_backup", ["test_save", backups[0]], {})
    assert_eq(loaded_backup.backup_test, test_data.backup_test, "Should load correct backup data")
    
    # Test backup rotation
    for i in range(10):
        TypeSafeMixin._safe_method_call_bool(save_manager, "create_backup", ["test_save", {"backup_number": i}])
    
    backups = TypeSafeMixin._safe_method_call_array(save_manager, "get_backups", ["test_save"], [])
    var max_backups: int = TypeSafeMixin._safe_method_call_int(save_manager, "get_max_backups", [], 0)
    assert_le(backups.size(), max_backups, "Should maintain maximum number of backups")
    
    # Test deleting backups
    success = TypeSafeMixin._safe_method_call_bool(save_manager, "delete_backups", ["test_save"], false)
...
func test_save_metadata() -> void:
    ...
    var success: bool = TypeSafeMixin._safe_method_call_bool(save_manager, "create_save_with_metadata",
        ["test_save", test_data, metadata], false)
    assert_true(success, "Should create save with metadata")
    
    # Test metadata retrieval
    var save_metadata: Dictionary = TypeSafeMixin._safe_method_call_dict(save_manager, "get_save_metadata", ["test_save"], {})
    ...
    success = TypeSafeMixin._safe_method_call_bool(save_manager, "update_save_metadata",
        ["test_save", updated_metadata], false)
    assert_true(success, "Should update save metadata")
    
    save_metadata = TypeSafeMixin._safe_method_call_dict(save_manager, "get_save_metadata", ["test_save"], {})
...
func test_save_compression() -> void:
    ...
    var success: bool = TypeSafeMixin._safe_method_call_bool(save_manager, "create_compressed_save",
        ["compressed_save", large_data], false)
    assert_true(success, "Should create compressed save")
    
    # Test compressed save loading
    var loaded_data: Dictionary = TypeSafeMixin._safe_method_call_dict(save_manager, "load_compressed_save",
        ["compressed_save"], {})
    assert_eq(loaded_data.array.size(), large_data.array.size(), "Should preserve all data in compressed save")
    
    # Test compression ratio
    var uncompressed_size: int = TypeSafeMixin._safe_method_call_int(save_manager, "get_uncompressed_size",
        ["compressed_save"], 0)
    var compressed_size: int = TypeSafeMixin._safe_method_call_int(save_manager, "get_compressed_size",
        ["compressed_save"], 0)
