...
func before_each() -> void:
    ...
    _override_manager = TypeSafeMixin._safe_cast_node(override_instance)
...
func test_override_registration() -> void:
    ...
    var result: bool = TypeSafeMixin._safe_method_call_bool(_override_manager, "register_override", [TEST_OVERRIDE_SETTINGS.BASIC])
    assert_true(result, "Should register basic override")
    verify_signal_emitted(_override_manager, "override_registered")
    
    var overrides: Dictionary = TypeSafeMixin._safe_method_call_dict(_override_manager, "get_overrides", [])
    assert_true(overrides.has("Basic Override"), "Should have basic override registered")

func test_override_enabling() -> void:
    watch_signals(_override_manager)
    
    TypeSafeMixin._safe_method_call_bool(_override_manager, "register_override", [TEST_OVERRIDE_SETTINGS.ADVANCED])
    var result: bool = TypeSafeMixin._safe_method_call_bool(_override_manager, "enable_override", ["Advanced Override"])
    assert_true(result, "Should enable advanced override")
    verify_signal_emitted(_override_manager, "override_state_changed")
    
    var is_enabled: bool = TypeSafeMixin._safe_method_call_bool(_override_manager, "is_override_enabled", ["Advanced Override"])
...
func test_override_settings() -> void:
    ...
    TypeSafeMixin._safe_method_call_bool(_override_manager, "register_override", [TEST_OVERRIDE_SETTINGS.BASIC])
    var settings: Dictionary = TypeSafeMixin._safe_method_call_dict(_override_manager, "get_override_settings", ["Basic Override"])
...
func test_invalid_override_handling() -> void:
    ...
    var result: bool = TypeSafeMixin._safe_method_call_bool(_override_manager, "register_override", [null])
    assert_false(result, "Should reject null override")
    verify_signal_not_emitted(_override_manager, "override_registered")
    
    result = TypeSafeMixin._safe_method_call_bool(_override_manager, "enable_override", ["NonexistentOverride"])
...
func test_override_performance() -> void:
    ...
    for i in range(100):
        ...
        TypeSafeMixin._safe_method_call_bool(_override_manager, "register_override", [override])
    
    var duration := Time.get_ticks_msec() - start_time
    assert_true(duration < 1000, "Should register 100 overrides within 1 second")
    verify_signal_emit_count(_override_manager, "override_registered", 100)