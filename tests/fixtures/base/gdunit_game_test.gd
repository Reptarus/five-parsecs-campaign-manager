...
func _initialize_game_systems() -> void:
	...
	# Only assert if we expect these to exist
...
# Test resource creation with metadata approach
func create_test_campaign(test_name: String = "Test Campaign") -> Resource:
	...
	campaign.set_meta("credits", 1000)
	campaign.set_meta("supplies", 50)
...
func create_test_character(test_name: String = "Test Character") -> Resource:
	...
	character.set_meta("character_name", test_name)
	character.set_meta("reactions", 1)
	character.set_meta("speed", 4)
	character.set_meta("combat_skill", 1)
	character.set_meta("toughness", 3)
	character.set_meta("savvy", 1)
...
# Safe assertion methods using metadata
func assert_campaign_valid(campaign: Resource, message: String = "") -> GdUnitObjectAssert:
	...
	assert_that(campaign.has_meta("campaign_name")).is_true()
	assert_that(campaign.has_meta("turn_number")).is_true()
...
func assert_character_valid(character: Resource, message: String = "") -> GdUnitObjectAssert:
	...
	assert_that(character.get_meta("character_name", "")).is_not_equal("")
	assert_that(character.get_meta("reactions", 0)).is_greater_equal(0)
	assert_that(character.get_meta("speed", 0)).is_greater_equal(0)
	assert_that(character.get_meta("combat_skill", 0)).is_greater_equal(0)
	assert_that(character.get_meta("toughness", 0)).is_greater_equal(0)
	assert_that(character.get_meta("savvy", 0)).is_greater_equal(0)
...
func assert_mission_valid(mission: Resource, message: String = "") -> GdUnitObjectAssert:
	...
	assert_that(mission.get_meta("mission_type", "")).is_not_equal("")
	assert_that(mission.get_meta("difficulty", 0)).is_greater_equal(0)
...
func create_mock_node(node_type: String = "Node") -> Node:
	...
	node.name = "MockNode"
	return track_node(node)

func create_mock_resource(resource_type: String = "Resource") -> Resource:
	"""Create a mock resource for testing"""
	var resource: Resource
	match resource_type:
		"PackedScene": resource = PackedScene.new()
		_: resource = Resource.new()
	
	return track_resource(resource)
...
func validate_resource_integrity(resource: Resource) -> bool:
	...
	if script:
		...
		return not property_list.is_empty()
...
func generate_test_data(data_type: String, count: int = 1) -> Array:
	...
	for i: int in range(count):
		match data_type:
			"campaign": data.append(create_test_campaign("Test Campaign %d" % (i + 1)))
			"character": data.append(create_test_character("Test Character %d" % (i + 1)))
			"mission": data.append(create_test_mission("Test Mission %d" % (i + 1)))
			_:
				data.append({"id": i, "name": "Test Data %d" % (i + 1)})
