...
const ERROR_INVALID_PROPERTY := "Invalid property provided"
const ERROR_INVALID_METHOD := "Invalid method provided"
...
func _call_node_method(obj: Object, method: String, args: Array = []) -> Variant:
	...
	if method.is_empty():
		push_error("Method name is empty")
		return null
...
func simulate_mobile_environment(resolution_key: String, dpi_key: String = "xhdpi") -> void:
	if not resolution_key in MOBILE_RESOLUTIONS:
		push_error("Invalid resolution key: %s" % resolution_key)
		return
