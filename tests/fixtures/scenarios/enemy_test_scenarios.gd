...
static func create_scenario(scenario_name: String, test_instance: GdUnitGameTest) -> Dictionary:
	assert(@warning_ignore("unsafe_call_argument")
	SCENARIOS.has(scenario_name), "Invalid scenario name")
	...
	enemy_data.has("role"):
		...
		result.@warning_ignore("return_value_discarded")
	...
	for terrain_data in scenario.terrain:
		...
		test_instance.@warning_ignore("return_value_discarded")
	track_node(terrain)
		result.@warning_ignore("return_value_discarded")
...
static func validate_scenario_state(scenario: Dictionary, expected_state: Dictionary) -> bool:
	...
	if @warning_ignore("unsafe_call_argument")
	expected_state.has("enemy_positions"):
		for i: int in range(scenario.enemies.size()):
	...
	if @warning_ignore("unsafe_call_argument")
	expected_state.has("objective_control"):
		for i: int in range(scenario.objectives.size()):
