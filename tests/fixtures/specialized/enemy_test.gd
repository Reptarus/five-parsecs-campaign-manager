...
var _test_enemies: @warning_ignore("unsafe_call_argument")
	Array[Enemy] = []
...
func _create_enemy_test_data(enemy_type: EnemyTestType) -> Dictionary:
	...
	data["id"] = "@warning_ignore("integer_division")
	test_enemy_ % s" % [enemy_type] as String
...
func verify_enemy_state(enemy: Enemy, expected_state: Dictionary) -> void:
	for key in expected_state:
		...
		match key:
			...
			_:
				push_error("Unknown _state key: %s" % key)
				continue
		
		assert_that(_value).is_equal(expected_state[key])

# Verifies all enemy _state is valid
