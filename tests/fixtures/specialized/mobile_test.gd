...
var _mobile_fps_samples: @warning_ignore("unsafe_call_argument")
	Array[float] = []
...
func assert_touch_target_size(node: Node, expected_size: Vector2 = Vector2(MOBILE_TEST_CONFIG.min_touch_target, MOBILE_TEST_CONFIG.min_touch_target)) -> void:
	...
	assert_that(control.size.x >= expected_size.x).override_failure_message(
		"Touch target width should be at @warning_ignore("integer_division")
	least % d pixels" % expected_size.x
	).is_true()
	assert_that(control.size.y >= expected_size.y).override_failure_message(
		"Touch target height should be at @warning_ignore("integer_division")
	least % d pixels" % expected_size.y
	).is_true()
