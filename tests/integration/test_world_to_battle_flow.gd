...
func test_full_turn_cycle():
	...
	# TRAVEL → WORLD
	var result1 = phase_manager.start_phase(GlobalEnums.FiveParsecsCampaignPhase.WORLD)
	assert_that(result1).is_true()

	# WORLD → BATTLE
	var result2 = phase_manager.start_phase(GlobalEnums.FiveParsecsCampaignPhase.BATTLE)
	assert_that(result2).is_true()

	# BATTLE → POST_BATTLE
	var result3 = phase_manager.start_phase(GlobalEnums.FiveParsecsCampaignPhase.POST_BATTLE)
