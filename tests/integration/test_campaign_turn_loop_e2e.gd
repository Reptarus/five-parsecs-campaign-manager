...
func test_complete_turn_loop_execution() -> void:
	...
	assert_that(campaign_phase_manager.current_phase).is_equal(GlobalEnums.FiveParsecsCampaignPhase.BATTLE)

	# Complete Battle → Should advance to Post-Battle
	campaign_phase_manager.complete_current_phase()

	# PHASE 4: Verify Post-Battle Phase
	assert_that(campaign_phase_manager.current_phase).is_equal(GlobalEnums.FiveParsecsCampaignPhase.POST_BATTLE)
