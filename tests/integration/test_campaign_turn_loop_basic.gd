...
## Week 4 - Campaign Turn Loop Basic Tests
## Tests phase transition rules and manager initialization (13 tests - safe limit)
...
func test_can_transition_from_travel_to_world() -> void:
	...
	var can_transition = campaign_phase_manager._can_transition_to_phase(GlobalEnums.FiveParsecsCampaignPhase.UPKEEP)
	assert_that(can_transition).is_true()

func test_can_transition_from_world_to_battle() -> void:
	campaign_phase_manager.force_phase_transition(GlobalEnums.FiveParsecsCampaignPhase.UPKEEP)
	var can_transition = campaign_phase_manager._can_transition_to_phase(GlobalEnums.FiveParsecsCampaignPhase.MISSION)
	assert_that(can_transition).is_true()

func test_can_transition_from_battle_to_post_battle() -> void:
	campaign_phase_manager.force_phase_transition(GlobalEnums.FiveParsecsCampaignPhase.MISSION)
	var can_transition = campaign_phase_manager._can_transition_to_phase(GlobalEnums.FiveParsecsCampaignPhase.POST_MISSION)
	assert_that(can_transition).is_true()

func test_can_transition_from_post_battle_to_travel() -> void:
	campaign_phase_manager.force_phase_transition(GlobalEnums.FiveParsecsCampaignPhase.POST_MISSION)
	var can_transition = campaign_phase_manager._can_transition_to_phase(GlobalEnums.FiveParsecsCampaignPhase.TRAVEL)
	assert_that(can_transition).is_true()
