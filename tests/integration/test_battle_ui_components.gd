...
## Tests signal flows between battle UI components via FPCM_BattleManager
## NOTE: FPCM_BattleEventBus was removed Feb 2026 (dead code).
## Tests that depended on it will skip gracefully.
...
# Test instances
# NOTE: event_bus was FPCM_BattleEventBus, removed Feb 2026 (dead code).
# Tests that relied on it will skip via is_instance_valid() guards.
...
func before_test() -> void:
	...
	# FPCM_BattleEventBus was removed (dead code, Feb 2026).
	# event_bus stays null — tests with is_instance_valid guards will skip.
	event_bus = null
...
func after_test() -> void:
	...
	# Wait for signal chains to complete
	for i in range(6):
